...
- 2026-02-23: [ID: P1-CED-01-S3] として hooks/profile ドキュメントへ kind 専用 hook 規約を反映した。`docs-jp/spec/spec-language-profile.md` に `on_render_expr_<kind>` の命名規則（CamelCase -> snake_case）と `render_expr` の優先順位（kind専用 -> kind汎用 -> leaf/complex -> 既定実装）を追記し、`docs-jp/spec/spec-dev.md` に同規約と TS（JS経由）適用を明記した。selfhost 回帰は `python3 tools/check_selfhost_cpp_diff.py --mode allow-not-implemented`（`mismatches=3` 既知基線）で確認した。
