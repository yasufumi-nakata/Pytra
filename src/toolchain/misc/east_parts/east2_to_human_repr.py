...
def render_east2_to_human_repr(out_doc: dict[str, Any]) -> str:
    """Render EAST doc into EAST2-oriented human-readable representation."""
    lines: list[str] = []
...
def render_east_to_human_repr(out_doc: dict[str, Any]) -> str:
    """Dispatch to stage-specific human-representation renderer."""
    east = out_doc.get("east")
    if out_doc.get("ok", False) and isinstance(east, dict) and east.get("east_stage") == 3:
        from .east3_to_human_repr import render_east3_to_human_repr

        return render_east3_to_human_repr(out_doc)
    return render_east2_to_human_repr(out_doc)


# Backward compatibility aliases.
def render_east2_human_cpp(out_doc: dict[str, Any]) -> str:
    return render_east2_to_human_repr(out_doc)
