...
def _runtime_discovery_repo_root() -> Path:
    cwd = Path.cwd()
...
_REPO_ROOT = _runtime_discovery_repo_root()
...
_RUNTIME_DISCOVERY_TYPE_ID_RUNTIME_NODE_KINDS: set[str] = {
    "IsInstance",
    "IsSubclass",
    "IsSubtype",
    "ClassDef",
...
_RUNTIME_DISCOVERY_STRING_OP_RUNTIME_SYMBOLS: set[str] = {
    "str.strip",
    "str.lstrip",
    "str.rstrip",
    "str.startswith",
...
def _runtime_discovery_sorted_strings(values: list[str]) -> list[str]:
    out: list[str] = []
    used: set[str] = set()
...
def _runtime_discovery_scan_runtime_refs(node: JsonVal, out: set[str], *, include_type_id_runtime: bool) -> None:
    """Collect embedded runtime_module_id references from lowered nodes."""
    if jv_is_list(node):
        for item in jv_list(node):
            _runtime_discovery_scan_runtime_refs(item, out, include_type_id_runtime=include_type_id_runtime)
        return
    ...
    if include_type_id_runtime and kind in _RUNTIME_DISCOVERY_TYPE_ID_RUNTIME_NODE_KINDS:
...
def _normalized_runtime_module_id(node: dict[str, JsonVal]) -> str:
    ...
    if runtime_module_id == "pytra.core.str":
        ...
        if runtime_symbol in _RUNTIME_DISCOVERY_STRING_OP_RUNTIME_SYMBOLS or runtime_call in _RUNTIME_DISCOVERY_STRING_OP_RUNTIME_SYMBOLS:
    ...
    *,
    ...
    while changed:
        ...
        for east_path in list(result.keys()):
            ...
            _runtime_discovery_scan_runtime_refs(
                east_doc,
