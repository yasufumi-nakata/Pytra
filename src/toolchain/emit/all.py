...
def _print_help() -> None:
    print(
        "usage: east2x.py INPUT.json --target {cpp,rs,cs,js,ts,go,java,kotlin,swift,ruby,lua,scala,php,nim} "
...
def _validate_east3_module(east: dict[str, Any]) -> dict[str, Any]:
    ...
    if int(stage_any) != 3:
        _fatal("invalid EAST stage: east2x accepts EAST3 only (east_stage=3)")
