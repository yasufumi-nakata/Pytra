...
def _is_math_runtime(expr: dict[str, Any]) -> bool:
    ...
    if symbol not in _KOTLIN_MATH_RUNTIME_SYMBOLS:
        return False
    if _has_runtime_extern_module(expr):
        return True
    runtime_call, _ = _resolved_runtime_call(expr)
...
def _render_attribute_expr(expr: dict[str, Any]) -> str:
    ...
    if semantic_tag.startswith("stdlib.") and runtime_call == "":
        raise RuntimeError("kotlin native emitter: unresolved stdlib runtime attribute: " + semantic_tag)
...
def _render_call_expr(expr: dict[str, Any]) -> str:
    ...
    if semantic_tag.startswith("stdlib.") and runtime_call == "":
        raise RuntimeError("kotlin native emitter: unresolved stdlib runtime call: " + semantic_tag)
    if runtime_call != "":
...
def _emit_swap(stmt: dict[str, Any], *, indent: str, ctx: dict[str, Any]) -> list[str]:
    ...
    tmp_type = _infer_kotlin_type(stmt.get("left"), _type_map(ctx))
    if tmp_type == "void":
        tmp_type = "Any?"
    return [
