...
class KotlinRenderer(CommonRenderer):
    ...
    def _emit_stmt(self, node: JsonVal) -> None:
        ...
        if kind == "AugAssign":
            ...
            if isinstance(target_node, dict) and self._str(target_node, "kind") == "Name":
                target = _safe_kotlin_ident(self._str(target_node, "id"))
            else:
    ...
    def _emit_expr(self, node: JsonVal) -> str:
        ...
        if kind == "Name":
            ...
            rendered = _safe_kotlin_ident(ident)
            rendered_type = self._render_type(resolved_type)
            if resolved_type not in ("", "unknown", "Any", "object") and rendered_type != "Any?":
                return "(" + rendered + " as " + rendered_type + ")"
            return rendered
        if kind == "Attribute":
