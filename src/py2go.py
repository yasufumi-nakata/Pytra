...
def _go_runtime_source_path() -> Path:
    """Go runtime 正本のソースファイルパスを返す。"""
    return Path(__file__).resolve().parent / "runtime" / "go" / "pytra" / "py_runtime.go"


def _copy_go_runtime(output_path: Path) -> None:
    """生成先ディレクトリへ Go runtime を配置する。"""
    runtime_src = _go_runtime_source_path()
    if not runtime_src.exists():
        raise RuntimeError("go runtime source not found: " + str(runtime_src))
    runtime_dst = output_path.parent / "py_runtime.go"
