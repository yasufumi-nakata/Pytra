#!/usr/bin/env python3
"""CLI helpers for toolchain2 Scala emit from linked manifest output."""
...
def emit_scala_from_manifest(manifest_path: Path, output_dir: Path) -> int:
    ...
    for item in modules.raw:
        ...
        if east_doc_obj is None:
            ...
            return 1
        code = emit_scala_module(east_doc_obj.raw)
        if code.strip() == "":
            continue
...
def main(argv: list[str]) -> int:
    ...
    while i < len(argv):
        ...
        if tok == "-h" or tok == "--help":
            print("usage: python3 src/toolchain2/emit/scala/cli.py MANIFEST.json [-o OUTPUT_DIR]")
            return 0
    ...
    if output_dir_text == "":
        output_dir_text = str(Path("work").joinpath("tmp").joinpath("emit").joinpath("scala"))
    return emit_scala_from_manifest(manifest_path, Path(output_dir_text))
