...
#ifndef PYTRA_GEN_BUILT_IN_STRING_OPS_H
#define PYTRA_GEN_BUILT_IN_STRING_OPS_H
...
list<str> py_split(const str& s, const str& sep, int64 maxsplit) {
    ...
    while (true) {
        ...
        if (at < 0)
            break;
        out.append(py_str_slice(s, pos, at));
        pos = at + m;
        splits++;
    }
    out.append(py_str_slice(s, pos, n));
    return out;
}
...
list<str> py_splitlines(const str& s) {
    ...
    while (i < n) {
        str ch = s[i];
        if ((ch == "\n") || (ch == "\r")) {
            out.append(py_str_slice(s, start, i));
            if ((ch == "\r") && (i + 1 < n) && (s[i + 1] == "\n"))
                i++;
            i++;
            start = i;
    ...
    if (start < n) {
        out.append(py_str_slice(s, start, n));
    } else if (n > 0) {
        auto __idx_1 = n - 1;
        str last = s[__idx_1];
        if ((last == "\n") || (last == "\r"))
...
#endif  // PYTRA_GEN_BUILT_IN_STRING_OPS_H
