...
// source: /workspace/Pytra/src/runtime/generated/std/argparse.east
// generated-by: src/toolchain/emit/cpp/cli.py

#ifndef PYTRA__WORKSPACE_PYTRA_SRC_RUNTIME_GENERATED_STD_ARGPARSE_H
#define PYTRA__WORKSPACE_PYTRA_SRC_RUNTIME_GENERATED_STD_ARGPARSE_H
...
struct ArgValue {
    ...
    struct _ArgSpec {
        ...
        _ArgSpec(const Object<list<str>>& names, const str& action = "", const Object<list<str>>& choices = rc_list_from_value(list<str>{}), const ArgValue& py_default = ArgValue{}, const str& help_text = "") {
            this->names = names;
            this->action = action;
    ...
    struct ArgumentParser {
        ...
        void add_argument(const str& name0, const str& name1 = "", const str& name2 = "", const str& name3 = "", const str& help = "", const str& action = "", const Object<list<str>>& choices = rc_list_from_value(list<str>{}), const ArgValue& py_default = ArgValue{}) {
        ...
        dict<str, ArgValue> parse_args(const ::std::optional<Object<list<str>>>& argv = ::std::nullopt) const {
            Object<list<str>> args;
            if (!argv.has_value())
                args = py_to<Object<list<str>>>(py_str_slice(pytra::std::sys::argv, 1, int64((pytra::std::sys::argv).size())));
            else
                args = py_to<Object<list<str>>>(argv.value());
            ...
            for (_ArgSpec s : rc_list_ref(this->_specs)) {
                if (s.action == "store_true") {
                    if ((s.py_default).tag == PYTRA_TID_BOOL)
                        values[s.dest] = s.py_default.unbox<bool, PYTRA_TID_BOOL>();
                    else
                        values[s.dest] = false;
...
using namespace pytra::std::argparse;
#endif  // PYTRA__WORKSPACE_PYTRA_SRC_RUNTIME_GENERATED_STD_ARGPARSE_H
