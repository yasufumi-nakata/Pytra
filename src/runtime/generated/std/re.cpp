...
namespace pytra::std::re {
    ...
    str group(const ::std::optional<Object<Match>>& m, int64 idx = 0) {
        /* `Match | None` から group を安全取得する（None は空文字）。 */
        if (!m.has_value())
            return "";
        Object<Match> mm = (m).value();
        return mm->group(idx);
    }
    
    str strip_group(const ::std::optional<Object<Match>>& m, int64 idx = 0) {
    ...
    ::std::optional<Object<Match>> match(const str& pattern, const str& text, int64 flags = 0) {
        // ^([A-Za-z_][A-Za-z0-9_]*)\[(.*)\]$
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)\\[(.*)\\]$") {
            ...
            if (!(_is_ident(head)))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{head, py_str_slice(text, i + 1, -(1))}));
        }
        if (pattern == "^def\\s+([A-Za-z_][A-Za-z0-9_]*)\\((.*)\\)\\s*(?:->\\s*(.+)\\s*)?:\\s*$") {
            ...
            if ((k >= t.size()) || (py_str_slice(t, k, k + 1) != "("))
                return ::std::nullopt;
            int64 r = py_to_int64(py_rfind(t, ")"));
            if (r <= k)
                return ::std::nullopt;
            str args = py_str_slice(t, k + 1, r);
            str tail = py_strip(py_str_slice(t, r + 1, int64(t.size())));
            if (tail == "")
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, args, ""}));
            if (!(py_startswith(tail, "->")))
                return ::std::nullopt;
            str ret = py_strip(py_str_slice(tail, 2, int64(tail.size())));
            if (ret == "")
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, args, ret}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)\\s*:\\s*([^=]+?)(?:\\s*=\\s*(.+))?$") {
            ...
            if (eq < 0) {
                ...
                if (ann == "")
                    return ::std::nullopt;
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, ann, ""}));
            }
            str ann = py_strip(py_str_slice(rhs, 0, eq));
            str val = py_strip(py_str_slice(rhs, eq + 1, int64(rhs.size())));
            if ((ann == "") || (val == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, ann, val}));
        }
        if (pattern == "^[A-Za-z_][A-Za-z0-9_]*$") {
            if (_is_ident(text))
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{}));
            return ::std::nullopt;
        }
        if (pattern == "^class\\s+([A-Za-z_][A-Za-z0-9_]*)(?:\\(([A-Za-z_][A-Za-z0-9_]*)\\))?\\s*:\\s*$") {
            ...
            if (tail == "")
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, ""}));
            if (!((py_startswith(tail, "(")) && (py_endswith(tail, ")"))))
                return ::std::nullopt;
            str base = py_strip(py_str_slice(tail, 1, -(1)));
            if (!(_is_ident(base)))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, base}));
        }
        if (pattern == "^(any|all)\\((.+)\\)$") {
            if ((py_startswith(text, "any(")) && (py_endswith(text, ")")) && (text.size() > 5))
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{"any", py_str_slice(text, 4, -(1))}));
            if ((py_startswith(text, "all(")) && (py_endswith(text, ")")) && (text.size() > 5))
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{"all", py_str_slice(text, 4, -(1))}));
            return ::std::nullopt;
        }
        if (pattern == "^\\[\\s*([A-Za-z_][A-Za-z0-9_]*)\\s+for\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+in\\s+(.+)\\]$") {
            ...
            if ((!(_is_ident(expr))) || (!(_is_ident(var))) || (it == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{expr, var, it}));
        }
        if (pattern == "^for\\s+(.+)\\s+in\\s+(.+):$") {
            ...
            int64 i = py_to_int64(py_find(rest, " in "));
            if (i < 0)
                return ::std::nullopt;
            ...
            if ((left == "") || (right == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{left, right}));
        }
        if (pattern == "^with\\s+(.+)\\s+as\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*:\\s*$") {
            ...
            int64 i = py_to_int64(py_rfind(rest, " as "));
            if (i < 0)
                return ::std::nullopt;
            ...
            if ((expr == "") || (!(_is_ident(name))))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{expr, name}));
        }
        if (pattern == "^except\\s+(.+?)\\s+as\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*:\\s*$") {
            ...
            if ((exc == "") || (!(_is_ident(name))))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{exc, name}));
        }
        if (pattern == "^except\\s+(.+?)\\s*:\\s*$") {
            ...
            if (rest == "")
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{rest}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*(?:\\.[A-Za-z_][A-Za-z0-9_]*)?)\\s*:\\s*(.+)$") {
            ...
            if ((ann == "") || (!(_is_dotted_ident(target))))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{target, ann}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*(?:\\.[A-Za-z_][A-Za-z0-9_]*)?)\\s*:\\s*([^=]+?)\\s*=\\s*(.+)$") {
            ...
            str target = py_strip(py_str_slice(text, 0, c));
            str rhs = py_str_slice(text, c + 1, int64(text.size()));
            int64 eq = py_to_int64(py_find(rhs, "="));
            if (eq < 0)
                return ::std::nullopt;
            str ann = py_strip(py_str_slice(rhs, 0, eq));
            ...
            if ((!(_is_dotted_ident(target))) || (ann == "") || (expr == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{target, ann, expr}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*(?:\\.[A-Za-z_][A-Za-z0-9_]*)?)\\s*(\\+=|-=|\\*=|/=|//=|%=|&=|\\|=|\\^=|<<=|>>=)\\s*(.+)$") {
            Object<list<str>> ops = rc_list_from_value(list<str>{"<<=", ">>=", "+=", "-=", "*=", "/=", "//=", "%=", "&=", "|=", "^="});
            int64 op_pos = -(1);
            str op_txt = "";
            for (str op : rc_list_ref(ops)) {
                auto p = py_find(text, op);
                if ((p >= 0) && ((op_pos < 0) || (p < op_pos))) {
                    op_pos = py_to_int64(p);
                    op_txt = op;
                }
            }
            if (op_pos < 0)
            ...
            if ((right == "") || (!(_is_dotted_ident(left))))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{left, op_txt, right}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)\\s*,\\s*([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(.+)$") {
            int64 eq = py_to_int64(py_find(text, "="));
            if (eq < 0)
                return ::std::nullopt;
            ...
            if (right == "")
                return ::std::nullopt;
            int64 c = py_to_int64(py_find(left, ","));
            if (c < 0)
                return ::std::nullopt;
            ...
            if ((!(_is_ident(a))) || (!(_is_ident(b))))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{a, b, right}));
        }
        ...
        if (pattern == "^([A-Za-z_][A-Za-z0-9_\\.]*)(?:\\s+as\\s+([A-Za-z_][A-Za-z0-9_]*))?$") {
            Object<list<str>> parts = rc_list_from_value(text.split(" as "));
            ...
            if ((rc_list_ref(parts)).size() == 2) {
                ...
                if ((!(_is_dotted_ident(name))) || (!(_is_ident(alias))))
                    return ::std::nullopt;
                return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, alias}));
            }
            return ::std::nullopt;
        }
        if (pattern == "^from\\s+([A-Za-z_][A-Za-z0-9_\\.]*)\\s+import\\s+(.+)$") {
            ...
            int64 i = py_to_int64(py_find(rest, " import "));
            if (i < 0)
                return ::std::nullopt;
            ...
            if ((!(_is_dotted_ident(mod))) || (sym == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{mod, sym}));
        }
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)(?:\\s+as\\s+([A-Za-z_][A-Za-z0-9_]*))?$") {
            Object<list<str>> parts = rc_list_from_value(text.split(" as "));
            if ((rc_list_ref(parts)).size() == 1) {
        ...
        if (pattern == "^([A-Za-z_][A-Za-z0-9_]*)\\s*:\\s*([^=]+?)\\s*=\\s*(.+)$") {
            ...
            if ((!(_is_ident(name))) || (ann == "") || (expr == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, ann, expr}));
        }
        ...
        if (pattern == "^type\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(.+)$") {
            ...
            if ((!(_is_ident(name))) || (rhs == ""))
                return ::std::nullopt;
            return ::make_object<Match>(0, text, rc_list_from_value(list<str>{name, rhs}));
        }
