 ...
 */
    ...
    void _png_append_list(Object<list<int64>>& dst, const Object<list<int64>>& src) {
        int64 i = 0;
    ...
    int64 _crc32(const Object<list<int64>>& data) {
    ...
    int64 _adler32(const Object<list<int64>>& data) {
        int64 mod = 65521;
        int64 s1 = 1;
        int64 s2 = 0;
        for (int64 b : rc_list_ref(data)) {
    ...
    Object<list<int64>> _png_u16le(int64 v) {
        return rc_list_from_value(list<int64>{v & 0xFF, v >> 8 & 0xFF});
    }
    
    Object<list<int64>> _png_u32be(int64 v) {
        return rc_list_from_value(list<int64>{v >> 24 & 0xFF, v >> 16 & 0xFF, v >> 8 & 0xFF, v & 0xFF});
    }
    
    Object<list<int64>> _zlib_deflate_store(const Object<list<int64>>& data) {
    ...
    Object<list<int64>> _chunk(const Object<list<int64>>& chunk_type, const Object<list<int64>>& data) {
        Object<list<int64>> crc_input = rc_list_from_value(list<int64>{});
    ...
    void write_rgb_png(const str& path, int64 width, int64 height, const bytes& pixels) {
        Object<list<int64>> raw = rc_list_from_value(list<int64>{});
        for (uint8 b : pixels) {
            rc_list_ref(raw).append(b);
        }
        ...
        Object<list<int64>> scanlines = rc_list_from_value(list<int64>{});
        int64 row_bytes = width * 3;
        int64 y = 0;
        while (y < height) {
            rc_list_ref(scanlines).append(0);
            int64 start = y * row_bytes;
            int64 end = start + row_bytes;
            int64 i = start;
            while (i < end) {
                rc_list_ref(scanlines).append(py_list_at_ref(rc_list_ref(raw), i));
                i++;
            }
            y++;
        }
        Object<list<int64>> ihdr = rc_list_from_value(list<int64>{});
        ...
        Object<list<int64>> idat = _zlib_deflate_store(scanlines);
        
        Object<list<int64>> png = rc_list_from_value(list<int64>{});
        ...
        Object<list<int64>> iend_data = rc_list_from_value(list<int64>{});
