...
1. [x] 未使用シンボル削除を「1シンボルずつ」進めた。
   ...
   - [x] 影響反映: public API を消した場合は `docs/spec-runtime.md` / `docs/spec-dev.md` の該当記述を同コミットで更新する。: 該当 API 記述が現行ドキュメントに存在しないことを確認済み（`rg` 検索）。
...
3. [x] 既存ユーザー向け移行手順を `docs/how-to-use.md` に追記する。
