...
- 2026-02-26: [ID: `P3-JAVA-NATIVE-01-S3-01`] Added parity-oriented lowering for list-comp(range) assignment (`__pytra_list_repeat` + range expansion), `min/max`, tuple destructuring/swap assignment, list truthiness in `if/while`, negative-index normalization, and `IfExp`. Re-ran `runtime_parity_check --case-root sample --targets java --all-samples --ignore-unstable-stdout` and improved to `pass=16/18` (remaining failures: `16_glass_sculpture_chaos`, `18_mini_language_interpreter`).
