...
- [ ] EAST3 の lowering で if/else ブロック内の変数宣言が hoist される
- [ ] EAST3 の lowering で for/while ブロック内の変数宣言が hoist される
- [ ] C++ emitter の既存 hoist ロジックを除去しても同じ C++ が生成される
- [ ] Dart emitter が hoist 済み EAST3 を受け取り、正しい Dart コードを生成する
- [ ] Zig/Julia emitter が同様に正しいコードを生成する

## 子タスク

- [ ] [ID: P0-BLOCK-SCOPE-VAR-HOIST-01-S1] if/else ブロック内の変数宣言 hoist を EAST3 lowering に実装する
- [ ] [ID: P0-BLOCK-SCOPE-VAR-HOIST-01-S2] for/while ブロック内の変数宣言 hoist を EAST3 lowering に実装する
- [ ] [ID: P0-BLOCK-SCOPE-VAR-HOIST-01-S3] C++ emitter の既存 hoist ロジックを EAST3 lowering に移行し、emitter から除去する
- [ ] [ID: P0-BLOCK-SCOPE-VAR-HOIST-01-S4] ユニットテストを追加する（if/else, for, while, nested blocks）
- [ ] [ID: P0-BLOCK-SCOPE-VAR-HOIST-01-S5] Dart/Zig/Julia emitter が hoist 済み EAST3 で正しく動作することを検証する
