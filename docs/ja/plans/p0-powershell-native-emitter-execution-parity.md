...
- 2026-03-22: Python 出力一致検証を導入。py_assert_stdout 正規実装、isinstance(__pytra_isinstance)、@property(decorators=['property'])、@dataclass フィールド初期化、enum クラス変数参照、format_value.east 生成+linker 暗黙依存、native seam 6 ファイル追加（C# パターン）、root_rel_prefix パス解決、extern passthrough スキップ、Cast/extern no-op、from pytra.std import X サブモジュール dot-source、クラス変数 $script: スコープ、super() チェーン修正、char メソッド [char]:: 変換、keyword-only 引数対応、str foreach .ToCharArray()、set foreach .Keys。Python 出力一致率 92/128 (71%) → 約 114/128 (89%)。
