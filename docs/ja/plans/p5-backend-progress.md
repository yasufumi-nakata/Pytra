...
- 2026-03-30: P5-PROGRESS S1〜S4 完了。`_save_parity_results()` を runtime_parity_check.py に実装し fast 版からも再利用。`tools/gen_backend_progress.py` で日英進捗ページを生成。README.md にリンク追加。selfhost 形式は計画書の定義通りで gen_backend_progress.py が selfhost_<lang>.json を読んでマトリクス表示する形で実装済み。
