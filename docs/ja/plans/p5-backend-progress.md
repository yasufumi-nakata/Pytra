...
`tools/gen/gen_backend_progress.py` が `work/parity-results/*.json` を読み、fixture 一覧・sample 一覧と突き合わせて Markdown を生成する。

出力先（日英同時生成。機械生成物を手動翻訳するのは非効率なため、スクリプトが両言語を直接出力する）:

- `docs/ja/language/backend-progress.md` — クッションページ（凡例 + 3マトリクスへのリンク + 関連リンク）。手書き。
- `docs/ja/language/backend-progress-fixture.md` — fixture parity マトリクス。機械生成。
- `docs/ja/language/backend-progress-sample.md` — sample parity マトリクス。機械生成。
- `docs/ja/language/backend-progress-selfhost.md` — selfhost マトリクス。機械生成。
- `docs/en/language/` にも同じ 4 ファイルを生成（クッションページの見出しだけ英語）。
...
[fixture / sample / selfhost の進捗マトリクス](docs/ja/language/backend-progress.md)
```
...
4. `docs/ja/language/backend-progress.md` が更新される
5. コミット・プッシュ
