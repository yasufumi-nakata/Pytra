...
- 2026-03-09 [ID: P1-EAST-TYPEEXPR-01-S5-01]: C++ の representative fail-closed lane として、`type_bridge._cpp_type_text()` と header builder が `int64|bool` のような general union を silent `object` fallback せず `ValueError("unsupported general union for C++ emit: ...")` で止めるようにした。`Any/object` 混じり dynamic union と optional は従来どおり互換を維持しつつ、general union だけを backend contract violation として明示化した。確認は `python3 -m py_compile src/backends/cpp/emitter/type_bridge.py src/backends/cpp/emitter/header_builder.py test/unit/backends/cpp/test_cpp_type.py test/unit/backends/cpp/test_py2cpp_features.py`、`PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_cpp_type.py'`、`PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_py2cpp_features.py' -k general_union_is_rejected_before_cpp_emit`、`PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_east3_cpp_bridge.py'`、`PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_py2cpp_features.py' -k json`、`python3 tools/check_todo_priority.py`、`git diff --check`。
