...
## Phase 2: emitter 層の @dataclass 化

Phase 1（parse/resolve/compile/link）完了を受け、emitter 層にも同じリファクタを適用する。C++ emitter を先行し、emit/common を先にやる。

### 対象と出現数

| ディレクトリ | 主なファイル | 出現数 | 備考 |
|---|---|---|---|
| `emit/common/` | `common_renderer.py` | 82 | 全 backend が import。EAST node 走査が大半だが、内部 context / render state に一時 dict がある |
| `emit/common/` | `cli_runner.py` | 9 | CLI 引数/設定の受け渡し |
| `emit/common/` | `profile_loader.py` | 6 | profile dict 構造 |
| `emit/cpp/` | `emitter.py` | 116 | 大半は EAST node 走査。内部の render context / type mapping に一時 dict がある |
| `emit/cpp/` | `header_gen.py` | 18 | ヘッダ生成の中間構造 |
| `emit/cpp/` | `runtime_bundle.py` | 11 | runtime ファイル収集 |

### サブタスク

#### S6: emit/common/ の @dataclass 化

`common_renderer.py` (82箇所) の内部 render state / context 構造を `@dataclass` に切り替える。EAST node 走査用の `dict[str, JsonVal]` 引数は非対象。

#### S7: emit/cpp/ の @dataclass 化

`emitter.py` (116箇所) の内部 render context / type mapping を `@dataclass` に切り替える。`header_gen.py` / `runtime_bundle.py` の中間構造も対象。

#### S8: 残り15言語の emitter の @dataclass 化

各 backend 担当が P1-EMITTER-SELFHOST-<LANG> の作業中に各自で移行する。共通パターンが S6/S7 で確立された後に着手。
