...
- [x] [ID: P1-RUNTIME-ABI-DECORATOR-01-S3-02] `py_join` など代表 helper を `@abi` 前提に移し、`rc<list<str>>` 非露出を回帰で固定する。
- [x] [ID: P1-RUNTIME-ABI-DECORATOR-01-S4-01] `@extern` 併用 case / unsupported backend / invalid mutation case の unit test を追加する。
- [x] [ID: P1-RUNTIME-ABI-DECORATOR-01-S4-02] docs 同期と `P1-CPP-PY-RUNTIME-SLIM-01` 依存解消メモを記録して本計画を閉じる。
...
- 2026-03-08: [ID: P1-RUNTIME-ABI-DECORATOR-01-S3-02] `src/pytra/built_in/string_ops.py` に `@abi(args={"parts": "value_readonly"}, ret="value")` 付き `py_join` を追加し、C++ generated runtime `src/runtime/cpp/generated/built_in/string_ops.{h,cpp}` へ `const list<str>&` / `list<str>` ABI の helper を materialize した。`runtime_symbol_index.json` と representative runtime regression も更新し、generated helper signature に `rc<list<str>>` が再侵入しないことを固定した。
- 2026-03-08: [ID: P1-RUNTIME-ABI-DECORATOR-01-S4-01] `runtime_abi` shared frontend に target support guard を追加し、`cpp` 以外では `@abi` を fail-closed にした。raw `ir2lang` と linked-program optimize の両方で同じ violation を返すようにし、parser は `@extern + @abi` metadata の共存、C++ backend はその併用時も value ABI lowering を維持する regression を追加した。`value_readonly` mutation case は `S2-02` で追加した validator tests を継続採用する。
- 2026-03-08: [ID: P1-RUNTIME-ABI-DECORATOR-01-S4-02] active TODO から本計画を閉じ、`P1-CPP-PY-RUNTIME-SLIM-01` 側へ「`py_join` は pure Python SoT + generated C++ helper + `@abi` lane で移行済み」という依存解消メモを残した。以後の `py_runtime` 縮退は fixed-ABI helper lane を前提に進められる。
