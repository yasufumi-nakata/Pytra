...
関連 TODO:
- `docs/ja/todo/index.md` の `ID: P1-IMPORT-GRAPH-ISSUE-STRUCTURE-01`

背景:
- import graph 解析は `relative_imports` / `missing_modules` / `cycles` などを `"<file>: <module>"` 形式の文字列 list で保持している。
- frontend 側では [transpile_cli.py](/workspace/Pytra/src/toolchain/frontends/transpile_cli.py) の `split_graph_issue_entry()` がその文字列を再分解しており、carrier と consumer が両方とも stringly-typed になっている。
- `P1-IMPORT-DIAGNOSTIC-STRUCTURE-01` で import diagnostic 自体は structured-first に寄せたため、次は import graph issue carrier も helper 境界へ寄せて brittle な `file: module` 再解釈を減らしたい。
...
- [ ] [ID: P1-IMPORT-GRAPH-ISSUE-STRUCTURE-01-S1-01] current import graph issue carrier と staged end state を棚卸しし、plan/TODO に固定する。
- [ ] [ID: P1-IMPORT-GRAPH-ISSUE-STRUCTURE-01-S2-01] graph issue entry の shared helper と focused unit test を追加し、`split_graph_issue_entry()` / producer 側の直列 formatting を helper 経由へ集約する。
- [ ] [ID: P1-IMPORT-GRAPH-ISSUE-STRUCTURE-01-S2-02] `validate_import_graph_or_raise()` / `format_import_graph_report()` / `east1_build` mirror を helper-structured seam に揃える。
- [ ] [ID: P1-IMPORT-GRAPH-ISSUE-STRUCTURE-01-S3-01] representative CLI / report regression、docs、archive を更新して閉じる。
