...
目的:
- `pytra-cli.py`（通常）/ `pytra-cli.py`（selfhost）を唯一の CLI 入口として確定する。
...
受け入れ基準:
- `src/` 直下の `py2*.py` は `pytra-cli.py` / `pytra-cli.py` のみ。
...
置換順（確定）:
1. `S2-01` で `tools` の direct wrapper 参照を `pytra-cli.py` / `pytra-cli.py` と backend module 参照へ更新する。
...
- [x] [ID: P1-PY2X-WRAPPER-REMOVE-REOPEN-01-S3-01] `src/py2*.py` wrapper 群と `toolchain/misc/py2x_wrapper.py` を削除する（`pytra-cli.py` / `pytra-cli.py` は除外）。
