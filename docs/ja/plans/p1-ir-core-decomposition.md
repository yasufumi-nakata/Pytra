...
- 2026-03-11: `core_stmt_text_semantics.py` を追加し、`_sh_split_top_level_assign` / `_sh_strip_inline_comment` / `_sh_raise_if_trailing_stmt_terminator` / `_sh_split_top_level_from` / `_sh_split_top_level_in` / `_sh_split_top_level_colon` / `_sh_parse_except_clause` / `_sh_parse_class_header_base_list` / `_sh_parse_class_header` を `core.py` から移した。statement/header text helper は `make_east_build_error` / `make_span` / `split_top_commas` を callback injection で受け取り、専用 source-contract test で固定する。
- 2026-03-11: `core_stmt_text_semantics.py` を logical-line merge / top-level split / comprehension target bind / indented-block collect まで拡張し、stmt-text helper の重複定義を `core.py` から除去した。source-contract は stmt-text helper の import/use/不在をまとめて検証する。
- 2026-03-11: `core_stmt_analysis.py` を追加し、`_sh_extract_leading_docstring` / `_sh_collect_yield_value_types` / `_sh_collect_return_value_types` / `_sh_infer_return_type_for_untyped_def` / `_sh_collect_store_name_ids` / `_sh_collect_reassigned_names` / `_sh_build_arg_usage_map` / `_sh_make_generator_return_type` を `core.py` から移した。statement analysis cluster は pure helper 群として dedicated source-contract test に分離する。
- 2026-03-11: `core_type_semantics.py` を追加し、`_sh_default_type_aliases` / `_sh_is_type_expr_text` / `_sh_typing_alias_to_type_name` / `_sh_register_type_alias` / `_sh_ann_to_type` / `_sh_ann_to_type_expr` / `_sh_type_expr_to_type_name` / `_sh_split_args_with_offsets` を `core.py` から移した。`_sh_set_parse_context` は `_SH_*` 更新を持つため core 側に残し、type helper cluster は dedicated source-contract test で固定する。
- 2026-03-11: `core_import_semantics.py` を追加し、`_sh_append_import_binding` / `_sh_import_binding_fields` / `_sh_make_import_resolution_binding` / `_sh_is_host_only_alias` / `_sh_register_import_symbol` / `_sh_register_import_module` を `core.py` から移した。登録 helper は import store と builder callback を受ける形へ変え、import-binding cluster は dedicated source-contract test で固定する。
- 2026-03-11: `core_import_module_builders.py` を追加し、`_sh_make_import_alias` / `_sh_make_import_binding` / `_sh_make_import_symbol_binding` / `_sh_make_qualified_symbol_ref` / `_sh_make_import_stmt` / `_sh_make_import_from_stmt` / `_sh_make_module_source_span` / `_sh_make_import_resolution_meta` / `_sh_make_module_meta` / `_sh_make_module_root` を `core.py` から移した。`Import` / `ImportFrom` / module root の代表 call site も split module 前提に更新し、builder cluster を source-contract test で固定する。
- 2026-03-11: `core_signature_semantics.py` を追加し、`_sh_parse_typed_binding` / `_sh_parse_augassign` / `_sh_parse_def_sig` を `core.py` から移した。`_sh_parse_def_sig` は `_SH_TYPE_ALIASES` / `_make_east_build_error` / `_sh_span` / `_sh_make_def_sig_info` を callback injection で受け、signature cluster は dedicated source-contract test で固定する。
- 2026-03-11: `core_builder_base.py` / `core_ast_builders.py` / `core_stmt_builders.py` を追加し、node/value/trivia primitive、expression builder、statement builder cluster を `core.py` から移した。`test_east_core_source_contract_builders.py` と `test_east_core_source_contract_stmt_builders.py` は helper 定義の所在と代表 call site を split module 前提で追う形に更新した。
- 2026-03-11: `core_expr_precedence.py` を追加し、`_parse_lambda` / `_parse_ifexp` / `or..unary` の precedence parser cluster を `core.py` から移した。`_ShExprParser` には `_raise_expr_build_error` だけを残し、source-contract は dedicated precedence test へ分離した。
- 2026-03-11: `core_entrypoints.py` を追加し、`EastBuildError` / `_make_east_build_error` / `convert_source_to_east*` / `convert_path` を `core.py` から移した。`core.py` は self-hosted parser 実装を残しつつ re-export だけを行い、`east_io.py` と `compiler/east_parts` は split module へ追従した。
- 2026-03-11: `core_parse_context.py` を parse/import/runtime/template context の正本に広げ、`_SH_IMPORT_SYMBOLS` / `_SH_IMPORT_MODULES` / `_SH_EMPTY_SPAN` / runtime ABI / template mode 定数も `core.py` から移した。source-contract は dedicated parse-context test に分離する。
- 2026-03-11: `core_parse_context.py` を追加し、`_SH_FN_RETURNS` / `_SH_CLASS_METHOD_RETURNS` / `_SH_CLASS_BASE` / `_SH_TYPE_ALIASES` / `_sh_set_parse_context` を `core.py` から移した。`core.py` は parse context を import して共有し、type helper / signature helper は既存の `_SH_TYPE_ALIASES` 利用箇所をそのまま維持する。
- 2026-03-11: `core_expr_parser_base.py` を追加し、`_tokenize` / `_cur` / `_eat` / `_node_span` / `_src_slice` / `parse` を `core.py` から移した。string token scan は `core_string_semantics.py` を直接参照し、source-contract は dedicated parser-base test へ分離した。
