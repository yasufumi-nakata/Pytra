# パイプライン再設計案: parse / resolve / optimize / emit 四段分離
...
## 2. 提案: parse / resolve / optimize / emit 四段パイプライン
...
optimize:  *.east2 → *.east3       EAST 最適化（whole-program）
...
```
pytra-cli -build --target=cpp a.py   （parse→resolve→optimize→emit を一気通貫）
```
...
```
pytra-cli -parse a.py        →  a.py.east1
pytra-cli -parse math.py     →  math.py.east1
```
...
```
pytra-cli -resolve --from=python *.py.east1  →  *.east2
```
...
#### optimize（言語非依存・whole-program）

```
pytra-cli -optimize *.east2  →  *.east3
```
...
- 責務:
  ...
  - type_id 判定・反復計画の命令化
  - dead code 除去
  - 現行の EAST3 lowering + linker optimizer に相当
- cross-module 依存: **あり**

#### emit（言語非依存→target・全モジュール一括）

```
pytra-cli -emit --target=cpp *.east3  →  *.cpp
pytra-cli -emit --target=rs  *.east3  →  *.rs
```
...
```
# 通常使用: parse→resolve→optimize→emit を一括
pytra-cli -build --target=cpp a.py

# build + compile + run まで一括
pytra-cli -run --target=cpp a.py
```
...
```
pytra-cli -parse pytra/built_in.py       →  built_in.py.east1
pytra-cli -parse pytra/std/math.py       →  math.py.east1
pytra-cli -parse pytra/std/json.py       →  json.py.east1
pytra-cli -parse user_code.py            →  user_code.py.east1
...
| optimize | `east2_to_east3_lowering.py` + linker optimizer | 現行 EAST3 lowering + linker optimizer を統合。 |
...
- one-shot コマンド `-run` の提供範囲（compile + execute まで含むか）
