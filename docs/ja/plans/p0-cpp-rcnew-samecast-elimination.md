...
- `src/hooks/cpp/emitter/expr.py`（cast 適用経路）
- `test/unit/test_py2cpp_smoke.py` 等の回帰テスト
...
- [ ] [ID: P0-CPP-RCNEW-SAMECAST-01-S1-01] `infer_rendered_arg_type()` で `::rc_new<T>(...)` を `rc<T>` と推論できる規約を追加する。
- [ ] [ID: P0-CPP-RCNEW-SAMECAST-01-S1-02] 同型 cast 省略判定（`should_skip_same_type_cast`）が上記推論結果を利用して `rc<T>(::rc_new<T>(...))` を no-op 判定できるようにする。
- [ ] [ID: P0-CPP-RCNEW-SAMECAST-01-S2-01] C++ cast 適用経路（`apply_cast` 周辺）で `rc_new` 起点の同型 cast が実際に落ちることを回帰テストで固定する。
- [ ] [ID: P0-CPP-RCNEW-SAMECAST-01-S2-02] `sample/cpp` を再生成し、`sample/18` の該当断片が `::rc_new<Token>(...)` へ簡約されることを確認する。
- [ ] [ID: P0-CPP-RCNEW-SAMECAST-01-S3-01] `check_py2cpp_transpile` と smoke を実行して非退行を確認し、決定ログへ結果を記録する。
