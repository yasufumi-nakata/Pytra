...
- 2026-03-13: `S4-01` の first bundle として、Wave B script backend に不足していた `generated/built_in/*` compare baseline を `js/ts/php` へ拡張した。`tools/runtime_generation_manifest.json` と `tools/gen_runtime_from_manifest.py` に JS/TS CJS built_in postprocess を追加し、`contains/io_ops/iter_ops/numeric_ops/predicates/scalar_ops/sequence/string_ops/type_id/zip_ops` を `js/ts/php` の generated compare target として固定した。`test_gen_runtime_from_manifest.py` では built_in manifest baseline を `rs/cs only` から `compare targets` 全体に引き上げ、`test_py2js_smoke.py` / `test_py2ts_smoke.py` / `test_py2php_smoke.py` では `generated/built_in/{contains,predicates,sequence}` の representative smoke を追加して repo tree からの direct-load / source check を固定した。
- 2026-03-13: `S4-01` の current bundle として、Wave B generated compare smoke inventory を contract/checker に追加した。`js/php` は `generated/built_in/{contains,predicates,sequence}` の repo-tree direct-load smoke、`ts` は `generated/built_in/{contains,sequence}` の source-guard smoke を baseline とし、representative built_in compare smoke の backend ごとの検証手段を `generated/native` vocabulary で固定した。
- 2026-03-13: `S4-01` の current bundle として、`go/java` の generated `built_in` compare baseline を manifest / contract に追加した。`contains/io_ops/iter_ops/numeric_ops/scalar_ops/zip_ops` を compile-safe subset として `generated/built_in/*` へ materialize し、`go` は runtime bundle compile smoke、`java` は small compile/run smoke、Wave A 全体は `source_guard` inventory で representative generated targets を固定した。これで static runtime family 側でも generated compare/helper lane の検証手段が `generated/native` vocabulary で揃った。
- 2026-03-13: `S4-01` の current bundle として、Wave A generated compare smoke inventory を contract/checker に分離した。`go/java` は `generated/built_in/contains.*` を `build_run_smoke`、`kotlin/scala/swift/nim` は helper-shaped `generated/utils/*` を `source_guard` として固定し、static runtime family の generated compare evidence taxonomy を `generated/native` vocabulary で明示した。
- 2026-03-13: `S4-01` の current bundle として、`kotlin/scala/nim` の generated `built_in` compare baseline を manifest / contract に追加した。`kotlin/scala` は `contains/iter_ops/predicates/sequence/zip_ops`、`nim` は `contains/iter_ops/numeric_ops/predicates/zip_ops` を compile-safe subset として `generated/built_in/*` へ materialize し、`tools/gen_runtime_from_manifest.py` には Kotlin/Scala の空 `main` を剥がす `program_to_library` postprocess を追加した。checker と tooling/backends smoke は `kotlin/nim = build_run_smoke`, `scala = source_guard` へ同期し、Wave A の compare-impossible backend は `swift` のみに縮退した。`swift` は native substrate に `__pytra_as_list` など generated built_in 側が要求する helper が不足しているため、この slice では blocked のまま据え置く。
