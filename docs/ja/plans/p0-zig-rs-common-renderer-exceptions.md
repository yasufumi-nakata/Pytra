...
- 2026-04-10: S4 終盤の棚卸しを実施。Zig の dispatch condition / handler guard condition / dispatch state stmt / handled mark stmt / propagation check も CommonRenderer に寄せ、冗長な `super()` pass-through override を削除した。現在 Zig emitter に残っている例外系 hook は、slot 名、exception match 条件、labeled block / `break :label`、fallback `with` protocol call、handler prelude/teardown といった Zig 構文 adapter にほぼ限定される。Rust 側は `With` の direct custom path を削除済みで、残る論点は `emit_custom_with_stmt()` を CommonRenderer 正本へ完全吸収するかどうかだけである。
