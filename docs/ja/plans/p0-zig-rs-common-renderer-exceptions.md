...
- 2026-04-10: Rust の `with` item loop もさらに縮小した。item metadata の bind 名 / enter target / exit metadata 解決、`with_enter_runtime_call`、`Rc<RefCell<...>>` fallback enter、context capture、bound 変数の新規宣言判定まで CommonRenderer helper に寄せている。残る Rust 固有部分は、loop 自体と `ctx.declared_vars` / `ctx.var_types` を使う状態更新・型判定の最終整理が中心である。
