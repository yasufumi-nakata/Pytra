...
- linker は trait を `type_id` 木から除外し、`trait_id_table_v1` / `class_trait_masks_v1` を生成し、`isinstance(x, Trait)` を `expected_trait_id` / `expected_trait_fqcn` 付き predicate に注釈する。
- C++ は trait を pure virtual class + `virtual public` 継承 + `override` + `__pytra_trait_bits` に写像し、`py_runtime_value_has_trait(...)` で判定する。
- Go は trait を interface に写像し、trait 継承は interface embedding、`isinstance(x, Trait)` は type assertion に写像する。
