...
- 2026-04-01: Lua emitter/runtime を継続修正。`boolop` truthiness、property getter、user-class `isinstance`、tuple-return unpack、dict/set comprehension、`range(1引数)`、union 経由 dict dispatch、`deque`/container `len()` を修正し、fixture は `125/137 pass`、stdlib は `16/16 pass` に改善。現行の残差は `class_tuple_assign`, `exception_bare_reraise`, `reversed_enumerate`, `str_index_char_compare`, `ok_class_inline_method`, `ok_fstring_format_spec`, `ok_lambda_default`, `callable_higher_order`, `object_container_access`, `str_repr_containers`。
