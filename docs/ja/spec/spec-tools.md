...
- 2026-03-08 snapshot では Wave 1 は `js/ts` が green、`rs/cs` が `toolchain_missing`、Wave 2 は `go/java/kotlin/swift/scala` が smoke/transpile green で parity pending、Wave 3 は `lua` が `target_smoke_fail`、`ruby/php/nim` が `transpile_fail` を primary failure とする。
