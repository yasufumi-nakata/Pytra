...
- `IfExp`: 真側と偽側の型を統合する。同型なら `T`、片側が `None` なら `OptionalType(inner=T)`、異なる非 None 型なら `UnionType`。

### 2.4.1 isinstance 型ナローイング

- `if isinstance(x, T):` の条件式を検出し、if ブロック（`body`）内で `x` の型を `T` に narrowing する。
- `elif isinstance(x, U):` も同様に処理する。
- `if not isinstance(x, T): return/raise/break/continue` の early return guard を検出し、後続の文で `x` を `T` に narrowing する。
- `y = x if isinstance(x, T) else default` の ternary isinstance を検出し、真側で `x` を `T` に narrowing する。
- narrowing は if ブロック内のループ等に自然に伝播する。
- narrowing は型環境の更新で実現し、`resolved_type` / `type_expr` を該当スコープで `T` に上書きする。
- if ブロック内で `x` に再代入がある場合、narrowing を無効化する（安全側）。
- v1 では `else` の除外型推論、`and`/`or` 組み合わせは対応しない。
- 詳細は [spec-east.md §7.1](./spec-east.md#71-isinstance-型ナローイングtype-narrowing) を参照。

### 2.5 cast 挿入

- 数値型の混在演算で自動昇格 cast を挿入する。
  - int-float 混在: `int64 + float64` → 左辺に `cast(int64 → float64)` を付与
  - int 同士のサイズ違い: `uint8 ^ int64` → `uint8` 側に `cast(uint8 → int64)` を付与
  - 昇格ルールは **C++ の integer promotion / usual arithmetic conversion に準ずる**:
    - 両辺のランク（型サイズ）が異なる場合、小さい方を大きい方に昇格
    - 同ランクで符号が異なる場合（`uint16 ^ int16` 等）、符号なし側に統一
