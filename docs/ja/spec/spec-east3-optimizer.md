...
### 8.4 `lifetime_analysis` 注釈契約（`east3_lifetime_v1`）

`LifetimeAnalysisPass` は `FunctionDef`（および `ClassDef` 内 method）へ次を注釈する。

- `meta.lifetime_analysis.schema_version = "east3_lifetime_v1"`
- `meta.lifetime_analysis.status`
  - `ok`
  - `fail_closed`（動的名前解決を検出）
- `meta.lifetime_analysis.reason`
  - `status=fail_closed` 時は `dynamic_name_access`
- `meta.lifetime_analysis.cfg`
  - ノード配列（`id`, `kind`, `defs`, `uses`, `succ`）
...
`lifetime_class` の規則（v1）:

- 既定: `local_non_escape_candidate`
- 次のいずれかで `escape_or_unknown`:
  - `status=fail_closed`
  - `Return/Yield` で当該変数が使用される
  - `escape_summary.arg_escape[i] == true` な引数

fail-closed 規則:

- `locals/globals/vars/eval/exec` の呼び出しを検出した関数は `status=fail_closed` にする。
- `fail_closed` な関数では lifetime 最適化候補を作らず、下流 backend は `escape_or_unknown` として扱う。
