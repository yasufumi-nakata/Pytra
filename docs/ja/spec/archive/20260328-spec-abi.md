...
- v2 では `extern_fn` / `extern_var` / `extern_class` に分離し、`module` / `symbol` / `tag` を必須引数とする。詳細は [spec-builtin-functions.md §10](./spec-builtin-functions.md) を参照。
