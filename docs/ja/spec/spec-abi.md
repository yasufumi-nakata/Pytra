...
Python 実行時互換のため、`@extern` 関数は Python で実行可能な本体（例: `return _m.sin(x)`）を持ってよい。
トランスパイラは `@extern` 関数の本体をターゲット実装としては採用せず、外部シンボル呼び出しへ lower する。

### 3.3 host-only import (`as _name`) 規約

`import ... as _m` のように alias が `_` で始まる import は host-only import として扱う。
...
- それ以外の箇所で `_m` 参照が出現した場合はコンパイルエラーとする。
...
`import ... as _name`（host-only import）の利用箇所制約は必須とする。
