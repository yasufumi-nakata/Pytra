...
本仕様において `@extern` は **引数なし**のみをサポートする。
...
@extern は Python 実行時に import を成立させるためのダミー定義として存在してよいが、意味論はトランスパイラが解釈する。
...
<module> は Python モジュール名（例: pytra.std.math, pytra.util.png 等）
...
pytra.util.write_png → pytra_util_write_png
...
bytes → rc<py_bytes>（実装依存）

int → py_int または int64_t（実装依存）
...
```
extern "C" void pytra_png_write_png(rc<list<rc<bytearray>>> image);
```
...
@extern 関数は Python 側に実装を持たず ... のスタブでよい。
...
runtime/cpp/pytra/std/math.cpp
は、トランスパイラが生成する。

@export を発見したので、このmathの事前に用意してあったランタイム(XXX-impl.cpp)も必要だとわかる。これは、
runtime/cpp/pytra/std/math-impl.cpp
に事前に配置しているものとする。

※ implがついているものは事前に配置したもの。それ以外は自動生成したものである。

manifestファイル(gccでのビルドに必要なファイルが書かれている)には、このとき、
