...
| 相対 import（`from .m import x`） | supported | sibling / parent relative `from-import` はサポート済み。`from .. import helper` の parent-package submodule binding も representative regression で固定済み。entry root を越える root escape は `input_invalid(kind=relative_import_escape)` で fail-closed。 | `test/unit/tooling/test_py2x_cli.py`, `test/unit/common/test_import_graph_issue_structure.py`, `test/unit/backends/cpp/test_py2cpp_features.py` |
