...
- next rollout handoff は [p1-relative-import-secondwave-planning.md](../plans/p1-relative-import-secondwave-planning.md) を参照し、`rs/cs/js/ts` を `transpile_smoke_locked` baseline に固定したうえで、残る non-C++ lane は `backend_specific_fail_closed` を維持しながら、`go/java/kotlin/nim/scala/swift` は `remaining_second_wave_rollout_planning`、`lua/php/ruby` は `defer_until_second_wave_remaining_complete` のまま進める。
