...
| Backend | [Progress](../en/language/backend-progress.md) | [進捗](./language/backend-progress.md) |
...
> **2026-03-28** — Go/C++ 例外処理完成。`@runtime` / `@extern` デコレータ設計完了。type_id table の linker 生成を実装。ガイド 5 ページ・チュートリアル 4 ページを追加。spec 再編（legacy 12 件アーカイブ）。

> **2026-03-27** — C++ emitter の spec 準拠完了（S1〜S15）。Trait（pure interface・多重実装）を導入し C++/Go で parity 達成。isinstance ナローイング（if/elif/early return guard/ternary）、三項演算子の Optional 型推論、pytra.std.json の parser 対応を完了。Lowering プロファイル + CommonRenderer の設計を開始。

[![全履歴](https://img.shields.io/badge/full_changelog-→-blue?style=flat-square)](./changelog.md)
