...
- 同じく `python3 tools/check_jsonvalue_decode_boundaries.py` も組み込まれているため、`pytra-cli` / `east_io` / `toolchain/link/*` の JSON artifact 境界で raw `json.loads(...)` が再侵入した場合は local CI で fail します。
