...
1. [x] [ID: P1-HOST-CPP-EMITTER-DART-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target dart -o work/selfhost/host-cpp/dart/` で変換 + build を通す
   ...
   - 完了: 2026-04-30。Docker Desktop CLI で `.devcontainer/Dockerfile` を直接 build した隔離環境にて、`timeout 240s python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target dart -o work/tmp/verify_pytra_20260430/host_cpp_dart` が PASS。18 source / 30 linked modules / Dart 25 files を出力し、`TupleUnpack` blocker は解消。devcontainer には `dart` CLI が無いため、生成 Dart の compile/parity は S2 blocker として残す。
