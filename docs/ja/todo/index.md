...
- [ ] [ID: P0-NONCPP-RUNTIME-GENERATED-CPP-BASELINE-01] non-C++ runtime generated lane を `cpp/generated/{built_in,std,utils}` baseline に揃える。進捗: `S2-01` を完了し、`rs/cs` の `generated/std/{argparse,json,random,re,sys,timeit}` と `generated/utils/assertions` を materialize した。さらに `S2-02` を完了し、static family（`go/java/kotlin/scala/swift/nim`）の `generated/{built_in,std,utils}` 25-module baseline を materialize して、Kotlin / Scala / Swift / Nim の `utils/{gif,png}` helper-shaped naming も canonical basename に寄せた。`S2-03` を完了し、script family（`js/ts/lua/ruby/php`）の `generated/{built_in,std,utils}` 25-module baseline を full materialize して、Lua の `ObjTypeId` / `pytra.std.sys` import / string_ops literal blocker を解消、current-target inventory / module buckets / manifest unit / backend smoke を canonical basename 前提へ同期した。`S3-01` では C# `math/json/pathlib` を generated-first へ切り替え、`time` backing seam を `native/std/time_native.cs` へ寄せたので、C# baseline std module に native canonical owner は残っていない。`S3-02` の first bundle では `generated/native/compat` の exact runtime file inventory を固定し、second bundle では baseline 外 `generated` helper inventory（現状は `utils/image_runtime` のみ）も contract に固定した。third bundle では checker / unit test の expected runtime inventory をこの contract の local inventory に一本化し、`remaining rollout` contract merge 依存を外した。文脈: [p0-noncpp-runtime-generated-cpp-baseline.md](../plans/p0-noncpp-runtime-generated-cpp-baseline.md)
