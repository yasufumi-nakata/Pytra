...
### P1-SPEC-CONFORM2: 仕様整合（Codex-review 追加指摘対応）

compile / link / optimize / emit が spec-linker.md / spec-emitter-guide.md の契約から逸脱している箇所を修正する。

1. [ ] [ID: P1-SPEC-CONFORM2-S1] type_id 構築: `bases` (複数) を正しく読む。未定義基底・循環継承を fail-fast にする（spec-linker §6.3）
2. [ ] [ID: P1-SPEC-CONFORM2-S2] linker/loader の fail-open を修正: runtime .east 読込失敗・不正 entry・欠損ファイル・不正 JSON をエラーにする（continue で握り潰さない）
3. [ ] [ID: P1-SPEC-CONFORM2-S3] manifest の `modules[*].input` を raw EAST3 入力パスに修正（spec-linker manifest 契約）
4. [ ] [ID: P1-SPEC-CONFORM2-S4] Go/C++ emitter から型推論・型変更・cast 追加を除去（spec-emitter-guide §1.1 禁止事項）。EAST 側で解決すべき問題は resolve/compile を修正
5. [ ] [ID: P1-SPEC-CONFORM2-S5] Go/C++ emitter の `pytra.` prefix / `math` 特別扱い等のハードコードを除去し、`mapping.json` に統一
6. [ ] [ID: P1-SPEC-CONFORM2-S6] `normalize_runtime_calls.py` の暫定互換レイヤを除去（toolchain2 emitter が canonical EAST3 を直接消費する）
7. [ ] [ID: P1-SPEC-CONFORM2-S7] optimizer/compile の resolved_type 後付け書き換えを修正（bytes subscript → int32 等の不整合）
8. [ ] [ID: P1-SPEC-CONFORM2-S8] cross-module default 引数展開のシンボル解決を module_id + receiver 型で修飾し、曖昧解決を防止
9. [ ] [ID: P1-SPEC-CONFORM2-S9] golden 再生成 + parity 維持確認
