...
文脈: [docs/ja/plans/p0-runtime-symbol-index-data-driven.md](../plans/p0-runtime-symbol-index-data-driven.md)
...
- 進捗メモ: 2026-03-06 [ID: `P0-RUNTIME-SYMBOL-INDEX-01-S1-01` / `-S1-02` / `-S2-01` / `-S2-02` / `-S2-03` / `-S3-01` / `-S3-02` / `-S3-03` / `-S4-02`] 直書き棚卸し・schema 固定・generator 追加・stale index guard・EAST3 `BuiltinCall` / import 解決済み symbol への `runtime_module_id/runtime_symbol` 付与・`signature_registry` の path 推定責務不在の guard・build tool の companion 導出 index 化まで実装した。
...
2. [ ] [ID: P0-CPP-REGRESSION-RECOVERY-01-S1-01] failing test を「generated runtime」「import/include 解決」「container 意味論」「emitter/CLI 契約」に再分類し、修正責務の所属レイヤを固定する。
