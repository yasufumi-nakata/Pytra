...
- [ ] [ID: P0-PYTRA-NES2-CROSSBACKEND-REPRO-01] Pytra-NES2 repro を全 backend representative contract に昇格する。進捗: `S1-01` で current repro inventory を固定済み。`bytes_truthiness` / `path_stringify` / `field_default_factory_rc_obj` は既存 representative lane に吸収済み、未解消は `property_method_call` と `list_bool_index`。README 記載の `path_alias_pkg/entry.py` は current bundle に現物がないため対象外とする。文脈: [p0-pytra-nes2-crossbackend-repro-contract.md](../plans/p0-pytra-nes2-crossbackend-repro-contract.md)
