...
- [ ] [ID: P0-NONCPP-RUNTIME-GENERATED-CPP-BASELINE-01] non-C++ runtime generated lane を `cpp/generated/{built_in,std,utils}` baseline に揃える。進捗: `S2-01` を完了し、`rs/cs` の `generated/std/{argparse,json,random,re,sys,timeit}` と `generated/utils/assertions` を materialize した。さらに `S2-02` を完了し、static family（`go/java/kotlin/scala/swift/nim`）の `generated/{built_in,std,utils}` 25-module baseline を materialize して、Kotlin / Scala / Swift / Nim の `utils/{gif,png}` helper-shaped naming も canonical basename に寄せた。`S2-03` first bundle では `php` の `generated/std/{argparse,glob,os,os_path,random,re,sys,timeit}` と `generated/utils/assertions` を materialize し、PHP keyword / `\\r` literal emitter bug を修正して baseline checker 上の materialized backend に昇格させた。second bundle では `js/ts` の missing 9 modules（`std/{argparse,glob,os,os_path,random,re,sys,timeit}` と `utils/assertions`）を `generated/` に materialize し、script family compare baseline / current-target inventory / module buckets / manifest unit / backend smoke を full 25-module 前提へ同期した。third bundle では Ruby の `generated/{built_in,std,utils}` 25-module baseline を materialize し、`utils/{gif,png}` を helper 名から canonical basename へ rename、baseline checker 上の materialized backend に昇格させた。加えて Ruby emitter の class-name rendering を正規化し、generated `std/argparse.rb` / `std/json.rb` を parse-safe 化した。残る script family full-baseline debt は Lua の blocker 解消である。文脈: [p0-noncpp-runtime-generated-cpp-baseline.md](../plans/p0-noncpp-runtime-generated-cpp-baseline.md)
