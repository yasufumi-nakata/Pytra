...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] active contract / checker / spec wording を `generated/native only` へ切り替え、checked-in `pytra/**` 再出現を fail-fast にする。進捗: fifth bundle で `rollout_remaining_contract` の lane ownership を `delete_target` へ反転した。schema 上の `target_roots=("generated","native","pytra")` と `compat_*` field 名はまだ残っている。
