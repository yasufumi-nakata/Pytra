...
1. [ ] [ID: P1-HOST-CPP-EMITTER-LUA-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target lua -o work/selfhost/host-cpp/lua/` で変換 + build を通す
   ...
   - 進捗: 2026-04-30 に `src/toolchain/emit/lua/cli.py` へ runtime copy を追加し、`src/toolchain/emit/lua/emitter.py` の entry `__script_dir` を global 化、table literal 直接 index を括弧付きに修正した。実行は `toolchain_emit_common_code_emitter.lua` の class body 内 `field()` 呼び出しで停止する。
