...
1. [ ] [ID: P1-HOST-CPP-EMITTER-LUA-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target lua -o work/selfhost/host-cpp/lua/` で変換 + build を通す
   ...
   - 進捗: 2026-04-30 に `src/toolchain/emit/lua/cli.py` へ runtime copy を追加し、`src/toolchain/emit/lua/emitter.py` の entry `__script_dir` global 化、table literal 直接 index の括弧付け、`field(default_factory=...)` lowering を実装した。実行は `toolchain_compile_jv.lua` の型 alias `Node = dict[str, JsonVal]` 相当が `dict` 未定義となり停止する。
