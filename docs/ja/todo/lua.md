...
5. [ ] [ID: P1-LUA-EMITTER-S5] fixture + sample の Lua run parity を通す（`lua5.4`）— 2026-04-02 現在 `fixture 144/144 pass`、`stdlib 16/16 pass`。pure-Python generated helper の load を emitter guide に沿って修正し、`pytra.utils.png/gif` を `dofile()` で接続。`bytearray.extend` と `dict.clear()` を runtime/emitter に追加して、新規 fixture を含む full fixture `144/144 pass` を再確認。sample は少なくとも `01_mandelbrot`, `03_julia_set`, `05_mandelbrot_zoom`, `17_monte_carlo_pi`, `18_mini_language_interpreter` の `PASS` を確認。未解消は主に sample 画像系で、`02_raytrace_spheres` / `04_orbit_trap_julia` は artifact CRC mismatch、`06_julia_parameter_sweep` / `07_game_of_life_loop` / `08_langtons_ant` は `--cmd-timeout-sec 600` で timeout。sample full の最新 pass 数は未確定
