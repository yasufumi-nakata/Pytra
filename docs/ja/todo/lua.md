...
最終更新: 2026-04-01（Lua fixture 131/137、stdlib 16/16、sample 再計測中）
...
5. [ ] [ID: P1-LUA-EMITTER-S5] fixture + sample の Lua run parity を通す（`lua5.4`）— 2026-04-01 現在の fixture は `131/137 pass`。今回 `callable_higher_order`, `exception_bare_reraise`, `ok_class_inline_method`, `str_index_char_compare` まで回復。残差は `class_tuple_assign`, `reversed_enumerate`, `ok_fstring_format_spec`, `ok_lambda_default`, `object_container_access`, `str_repr_containers`。sample は再計測中
