...
1. [x] [ID: P1-HOST-CPP-EMITTER-LUA-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target lua -o work/selfhost/host-cpp/lua/` で変換 + build を通す
   ...
   - 完了: 2026-04-29 `--target lua` の build dispatch を subprocess emitter へ再接続し、Docker `python:3.12-slim` 隔離環境で `timeout 180s python src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target lua -o work/tmp/verify_pytra_20260429/host_cpp_lua` が PASS。18 source / 30 linked modules / Lua 20 files を出力。
2. [ ] [ID: P1-HOST-CPP-EMITTER-LUA-S2] C++ emitter host parity PASS を確認し、結果を `.parity-results/emitter_host_lua.json` に書き込む（`gen_backend_progress.py` で emitter host マトリクスに反映される）
   ...
   - 進捗: 2026-04-29。変換は PASS したが生成 Lua emitter の実行 parity は未実施。この macOS 環境には `lua` / `luac` がないため、devcontainer または Lua 導入済み環境で再開する。
