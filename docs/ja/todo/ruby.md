...
1. [ ] [ID: P1-HOST-CPP-EMITTER-RUBY-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target ruby -o work/selfhost/host-cpp/ruby/` で変換 + build を通す
   ...
   - 進捗: 2026-04-30 に `src/toolchain/emit/ruby/cli.py` へ runtime copy を追加し、`src/toolchain/emit/ruby/emitter.py` で `field(default_factory=...)` を Ruby 初期値へ lowering した。`ruby -c work/selfhost/host-cpp/ruby/toolchain_emit_cpp_cli.rb` は `Syntax OK`。実行は `toolchain_compile_jv.rb` の型 alias `Node = __pytra_get_index(dict, __pytra_tuple([str, JsonVal]))` で `dict` 未定義となり停止する。
