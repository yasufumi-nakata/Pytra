...
2. [x] [ID: P1-HOST-CPP-EMITTER-NIM-S2] C++ emitter host parity PASS を確認し、結果を `.parity-results/emitter_host_nim.json` に書き込む（`gen_backend_progress.py` で emitter host マトリクスに反映される）
   ...
   - 2026-04-29: `.parity-results/emitter_host_nim.json` に build/parity PASS を記録。`gen_backend_progress.py` で emitter host マトリクスに反映済み。
