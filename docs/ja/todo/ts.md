...
2. [x] [ID: P1-HOST-CPP-EMITTER-TS-S2] C++ emitter host parity PASS を確認し、結果を `.parity-results/emitter_host_ts.json` に書き込む（`gen_backend_progress.py` で emitter host マトリクスに反映される）
   ...
   - 2026-04-29: `diff -ru work/selfhost/host-cpp/python work/selfhost/host-cpp/ts-run` の一致を再確認し、`.parity-results/emitter_host_ts.json` に build/parity PASS を記録。`gen_backend_progress.py` で emitter host マトリクスに反映済み。
