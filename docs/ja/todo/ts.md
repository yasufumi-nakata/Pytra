...
1. [ ] [ID: P1-HOST-CPP-EMITTER-TS-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target ts -o work/selfhost/host-cpp/ts/` で変換 + build を通す
2. [ ] [ID: P1-HOST-CPP-EMITTER-TS-S2] ts 版 C++ emitter で fixture manifest を処理し、Python 版 emitter と parity 一致を確認する
