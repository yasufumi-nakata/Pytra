...
- The source of truth for folder boundaries is [`docs/en/spec/spec-folder.md`](./spec-folder.md); this document focuses on implementation behavior.
...
## 3. C++ Conversion Spec (`py2x.py --target cpp`)

- Parses Python AST and generates a single `.cpp` (with required includes).
- Detailed support granularity (`enumerate(start)`, `lambda`, comprehensions, etc.) is managed in `docs/en/language/cpp/spec-support.md`.
...
- Class-method calls are routed via dispatch mode separation in `src/backends/cpp/emitter/call.py` (`virtual` / `direct` / `fallback`).
...
`py2x.py --target cpp` emits includes according to import statements.
...
- Recommended checks:
  - Check source diff between version generated by `src/backends/cpp/cli.py` and version generated by `selfhost` (diff itself is allowed).
...
- Responsibility separation:
  ...
  - `src/backends/cpp/cli.py` stringifies normalized EAST
...
- Canonical detailed schema is `docs/en/spec/spec-language-profile.md`.
