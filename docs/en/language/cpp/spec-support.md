...
| Relative import (`from .m import x`) | supported | Sibling and parent relative `from-import` are supported. Parent-package submodule binding via `from .. import helper` is also locked by representative regression. Root-escape cases that climb above the entry root stay fail-closed with `input_invalid(kind=relative_import_escape)`. | `test/unit/tooling/test_py2x_cli.py`, `test/unit/common/test_import_graph_issue_structure.py`, `test/unit/backends/cpp/test_py2cpp_features.py` |
