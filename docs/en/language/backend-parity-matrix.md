...
- Today `cpp` is `build_run_locked`, while `rs/cs/go/java/js/kotlin/nim/scala/swift/ts` are `transpile_smoke_locked`. The evidence lane stays fixed as direct native-emitter function-body transpile smoke for `go/nim/swift`, and as `package_project_transpile` for the JVM package bundle on `java/kotlin/scala`. `lua/php/ruby` remain `not_locked`.
...
- The next rollout handoff lives in [p1-relative-import-jvm-package-bundle.md](../plans/p1-relative-import-jvm-package-bundle.md): the bundle order is fixed as `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle`. `java/kotlin/scala` have already locked representative smoke, but the active `jvm_package_bundle_rollout` still owns the parity-doc and handoff closeout. The follow-up `lua/php/ruby` lane stays on the `longtail_relative_import_rollout` and remains deferred behind `defer_until_jvm_package_bundle_complete`. The current non-C++ fail-closed lane stays explicitly `backend_specific_fail_closed`.
