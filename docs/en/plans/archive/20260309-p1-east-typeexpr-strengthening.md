...
- [x] [ID: P1-EAST-TYPEEXPR-01-S6-01] Put a representative `JsonValue` lane on top of the new `TypeExpr` / nominal-ADT contract and verify that future runtime work can proceed IR-contract-first.
...
- 2026-03-09 [ID: P1-EAST-TYPEEXPR-01-S6-01]: Kept the representative lane fixed to `json.value.as_obj`, but moved semantic inference off `owner_type == "JsonValue"` and onto `TypeExpr`/nominal summaries. `summarize_type_expr()` now carries `nominal_adt_name`, `json_decode_v1` now records `receiver_nominal_adt_name`, and a new regression proves the lane still lowers when `resolved_type="unknown"` but `type_expr=JsonValue` is present. The migration-only compat path is now also labeled explicitly as `contract_source=resolved_type_compat`. The inverse mismatch now stops with `json_decode_contract_violation` when `semantic_tag=json.value.as_obj` disagrees with the receiver nominal name. This keeps later runtime nominalization work anchored on IR metadata instead of raw method names or string mirrors. Verified with `python3 -m py_compile src/toolchain/frontends/type_expr.py src/toolchain/ir/east2_to_east3_lowering.py test/unit/common/test_frontend_type_expr.py test/unit/ir/test_east2_to_east3_lowering.py`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/common -p 'test_frontend_type_expr.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/ir -p 'test_east2_to_east3_lowering.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/common -p 'test_pylib_json.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_east3_cpp_bridge.py'`, `python3 tools/check_todo_priority.py`, and `git diff --check`.
- 2026-03-09 [ID: P1-EAST-TYPEEXPR-01-S6-02]: Expanded the reintroduction guard beyond the representative lane to cover `json.obj.get_int`, `json.arr.get_bool`, and `json.loads_obj`, locking `receiver_nominal_adt_name`, `result_type.nominal_adt_name`, and `decode_kind=module_load` in `tools/check_jsonvalue_typeexpr_contract.py`. The tool import path was also normalized to `toolchain.*` so it stays stable both from repo-root execution and selfhost-adjacent module layouts. Verified with `python3 tools/check_jsonvalue_typeexpr_contract.py` and `PYTHONPATH=src python3 -m unittest discover -s test/unit/tooling -p 'test_check_jsonvalue_typeexpr_contract.py'`.
