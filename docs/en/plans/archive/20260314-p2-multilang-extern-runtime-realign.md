...
- [x] [ID: P2-MULTILANG-EXTERN-RUNTIME-REALIGN-01-S2-01] Remove module-specific extern rewrites from `tools/runtime_generation_manifest.json` and `tools/gen_runtime_from_manifest.py`, and align generated lanes to declaration/wrapper-only output.
- [x] [ID: P2-MULTILANG-EXTERN-RUNTIME-REALIGN-01-S2-02] Establish canonical extern-backed owners under `src/runtime/<lang>/native/**` for each target and synchronize the runtime symbol index plus layout contracts.
...
- [x] [ID: P2-MULTILANG-EXTERN-RUNTIME-REALIGN-01-S3-01] Sync representative runtime artifacts, smoke tests, docs, and contract inventories to the current extern-ownership contract and close the task.
...
- 2026-03-14: Also folded the Java `std/time` manifest hook from `java_perf_counter_host_wrapper` into `java_std_native_owner_wrapper + helper_name=time`. The artifact shape stays the same, but the Java lane now follows the same “generic owner-wrapper, no module-specific postprocess name” contract as the other migrated targets.
...
- 2026-03-14: Applied the same split to PHP `std/time`, removing `microtime(true)` from `generated/std/time.php` and turning it back into a wrapper over `src/runtime/php/native/std/time_native.php`. The staged runtime hook metadata, Wave B rollout/baseline contracts, inventory, PHP smoke suite, and `check_py2x_profiles.json` now all assume that `time_native.php` seam.
- 2026-03-14: Applied the same split to JS/TS `std/sys`, removing direct `process.argv` / `process.stderr` host bindings from `generated/std/sys.{js,ts}` and turning those files back into wrappers over `src/runtime/js|ts/native/std/sys_native.*`. The manifest now uses `js|ts_std_native_owner_wrapper + helper_name=sys`, and the inventory, rollout/baseline contracts, plus JS/TS smoke tests are synchronized around that sys seam.
- 2026-03-14: Added another `S2-01` bundle to rename the remaining Rust `std/math` hook from `rs_std_math_live_wrapper` to `rs_math_runtime_wrapper`, while folding the PHP lane into `php_std_native_owner_wrapper + helper_name=math`. The manifest, generator, inventory, and unit tests now use generic math-runtime seam naming.
- 2026-03-14: Applied the same split to PHP `std/math`, removing direct `pyMath*` calls from `generated/std/math.php` and turning it back into a wrapper over `src/runtime/php/native/std/math_native.php`. The native seam now carries the missing `log10` helper as well, and the staged runtime metadata, Wave B rollout/baseline contracts, inventory, PHP smoke tests, and `check_py2x_profiles.json` have all been synchronized around `math_native.php`.
- 2026-03-14: Added one more `S2-01` slice to fold the JS/TS `std/time` manifest hook names from `js_perf_counter_host_wrapper` / `ts_perf_counter_host_wrapper` into `js|ts_std_native_owner_wrapper + helper_name=time`. The generic owner-wrapper now carries the `perfCounter` alias and the TS `number` return annotation, so the artifact shape stays stable while the module-specific wrapper names disappear.
- 2026-03-14: The Rust lane had already moved toward the split between `rs_std_native_owner_wrapper + helper_name=time|math` and `native/std/time_native.rs` / `native/std/math_native.rs`, so the inventory, layout/baseline contracts, runtime-hook metadata, and unit-test expectations were synchronized with the current artifact shape. The C# alias guard was also updated to match the current extern-owner resolution path instead of looking for a direct `Pytra.CsModule.math` return.
- 2026-03-14: Closed `S2-01/S2-02` by adding `noncpp_ownership_mode` plus `accepted_generated_compare_residual_targets` to the inventory. `std/math,time,sys` are now explicitly locked as live native-owner lanes, while `std/os,os_path,glob` plus `built_in/io_ops,scalar_ops` are explicitly locked as accepted generated-compare residuals. This makes the manifest/generator rewrite cleanup and the native-owner/current-residual policy share the same checker-backed contract.
- 2026-03-14: Closed `S3-01` by synchronizing the representative smoke inventory/checker, native-seam wrapper evidence for JS/TS/C#/Java/PHP/Rust, the docs mirror, and TODO/archive handoff to the current extern-ownership contract. The tracked `@extern` modules are now explicitly fixed to one of two end states: `generated wrapper + native owner` or `accepted generated-compare residual`.
