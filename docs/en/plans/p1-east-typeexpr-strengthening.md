...
- 2026-03-09 [ID: P1-EAST-TYPEEXPR-01-S5-01]: As the representative C++ fail-closed lane, `type_bridge._cpp_type_text()` and the header builder now stop on general unions such as `int64|bool` with `ValueError("unsupported general union for C++ emit: ...")` instead of silently collapsing them to `object`. Dynamic unions containing `Any/object` and optionals keep their current compatibility path; only true general unions are now treated as backend-contract violations. Verified with `python3 -m py_compile src/backends/cpp/emitter/type_bridge.py src/backends/cpp/emitter/header_builder.py test/unit/backends/cpp/test_cpp_type.py test/unit/backends/cpp/test_py2cpp_features.py`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_cpp_type.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_py2cpp_features.py' -k general_union_is_rejected_before_cpp_emit`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_east3_cpp_bridge.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/backends/cpp -p 'test_py2cpp_features.py' -k json`, `python3 tools/check_todo_priority.py`, and `git diff --check`.
