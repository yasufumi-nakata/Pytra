<a href="../../ja/plans/p0-cpp-s18-rc-copy-reduction.md"><img alt="Read in Japanese" src="https://img.shields.io/badge/docs-日本語-2563EB?style=flat-square"></a>

> [!WARNING]
> This file is synchronized from `docs/ja/plans/p0-cpp-s18-rc-copy-reduction.md` and still requires manual English translation.

> Source of truth: `docs/ja/plans/p0-cpp-s18-rc-copy-reduction.md`
...
- 2026-03-01: S2-03 として `AnnAssign/Assign` の subscript 由来 `rc` 一時束縛を安全条件付きで `const` 参照化し、sample/18 で `const rc<ExprNode>& node = expr_nodes[expr_index];` 反映を確認した。
...
- [x] [ID: P0-CPP-S18-RC-COPY-02-S2-03] 添字アクセス由来の一時 `rc` 束縛（例: `node = expr_nodes[idx]`）を、安全条件付きで `const` 参照へ縮退する。
