...
- Progress memo: The tenth `S2-02` bundle is done. `S2-01` is complete with the emitter reduced to a helper-only residual, and regenerating `iter_ops/gif` plus upstreaming the typed empty-bytes lane retires the generated boxed-list seed bucket. The sample `py_append` bucket and the generated/sample generic-index buckets are retired too, and routing ref-first typed-list subscripts through `py_list_at_ref(rc_list_ref(...), ...)` leaves the typed-lane residual inventory with only the emitter helper bucket. Next is `S2-03`: shrinking typed-path fallback around generic `make_object` / `py_to` / dict-key coercion.
