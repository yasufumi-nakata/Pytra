...
- [ ] [ID: P0-NONCPP-RUNTIME-LAYOUT-ALIGN-01] Re-layout the `rs/cs` runtimes into a `generated/native` tree that can be compared against C++ at file level (context: [p0-noncpp-runtime-layout-alignment.md](../plans/p0-noncpp-runtime-layout-alignment.md)). Progress: the `rs/cs` trees are already cut over to `native/generated`, and `png/gif` plus `std/{time,math,os,os_path,glob,pathlib}` have been regenerated there from SoT. The C# std ownership contract is fixed as `json=native(std, generated blocked)`, `pathlib/math=native canonical + generated compare artifact`, and `re/argparse/enum=no runtime module`; the Rust std ownership contract is fixed as `time/math=native(built_in)`, `pathlib/os/os_path/glob=compare artifact only`, `json blocked`, and `re/argparse/enum=no runtime module`.
- [ ] [ID: P1-NONCPP-RUNTIME-LAYOUT-ROLLOUT-REMAINING-01] Roll the `go/java/kotlin/scala/swift/nim/js/ts/lua/ruby/php` runtimes onto the same `generated/native` ownership model as P0 (context: [p1-noncpp-runtime-layout-rollout-remaining.md](../plans/p1-noncpp-runtime-layout-rollout-remaining.md)). Progress: added the rollout plan that extends the P0 compare unit `<lane>/<bucket>/<module>` and the `generated=SoT only`, `native=hand-written only` rule to the remaining backends.
