...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] Switch active contracts / checkers / spec wording to `generated/native only` and make checked-in `pytra/**` re-entry fail fast. Progress: the seventh bundle replaced the wave-B `compat` helper / iterator naming in `rollout_remaining_contract` with `delete_target`. The main active contract residue now lives in `noncpp_runtime_generated_cpp_baseline_contract.py` as `compat_files`.
