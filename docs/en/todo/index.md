...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] Switch active contracts / checkers / spec wording to `generated/native only` and make checked-in `pytra/**` re-entry fail fast. Progress: the fourth bundle moved the `rollout_remaining_contract` `pytra` rationale and checker message toward `delete-target debt` wording. The schema-level `target_roots/compat_*` taxonomy still remains.
