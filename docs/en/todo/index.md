...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] Switch active contracts / checkers / spec wording to `generated/native only` and make checked-in `pytra/**` re-entry fail fast. Progress: the sixth bundle shrank the live `target_roots` in `rollout_remaining_contract` to `("generated","native")`, and the checker now only allows `pytra/**` under `delete_target` lanes. `compat_files/compat_modules` and the wave-B `compat` helper names still remain.
