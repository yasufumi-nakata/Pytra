...
- [ ] [ID: P1-NONCPP-RUNTIME-LAYOUT-ROLLOUT-REMAINING-01] Roll the `go/java/kotlin/scala/swift/nim/js/ts/lua/ruby/php` runtimes onto the same `generated/native` ownership model as P0 (context: [p1-noncpp-runtime-layout-rollout-remaining.md](../plans/p1-noncpp-runtime-layout-rollout-remaining.md)). Progress: `S1-01` locked the mapping table and hook/path baseline checker, and `S1-02` locked the current file inventory baseline in its first bundle and the target inventory baseline in its second bundle.
