...
## 2026-03-01 Migration: Completed sections (6 items)

The following items were moved from `docs/ja/todo/index.md` to `todo/archive`.

### P0: EAST3-led safe `reserve` optimization (unconditional-append only)

Context: [docs/ja/plans/p0-east3-safe-reserve-hint.md](../plans/p0-east3-safe-reserve-hint.md)

1. [x] [ID: P0-EAST3-SAFE-RESERVE-01] Enable `reserve` only when append is unconditional and the loop count is known in advance, and move the judgment responsibility to EAST3.
2. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S1-01] Specify the ForCore conditions in EAST3 that qualify for `reserve` (unconditional append / static range / invariant stop).
3. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S1-02] Define the EAST3 `reserve` hint data shape (owner, count expression, safety flag).
4. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S2-01] Add an EAST3 optimizer pass that attaches `reserve` hints only to eligible loops.
5. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S2-02] Remove `capture`-derived `reserve` estimation from the C++ emitter and switch to EAST3 hint references.
6. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S3-01] Fix non-output of inappropriate `reserve` in regenerated `sample/18` and update regression tests.
7. [x] [ID: P0-EAST3-SAFE-RESERVE-01-S3-02] Confirm non-regression with `test_py2cpp_codegen_issues.py` / `check_py2cpp_transpile.py`.

### P0: Further reduction of same-type cast chains in sample/18 C++

Context: [docs/ja/plans/p0-cpp-s18-samecast-chain-reduction.md](../plans/p0-cpp-s18-samecast-chain-reduction.md)

1. [x] [ID: P0-CPP-S18-SAMECAST-02] Reduce same-type cast chains such as `int64(py_to<int64>(...))` in sample/18.
2. [x] [ID: P0-CPP-S18-SAMECAST-02-S1-01] Inventory redundancy points (dict fetches / helper return values) and fix reduction targets.
3. [x] [ID: P0-CPP-S18-SAMECAST-02-S2-01] Update EAST3 cast cleanup and C++ emitter conditions to reduce same-type cast chains.
4. [x] [ID: P0-CPP-S18-SAMECAST-02-S2-02] Add regression tests to detect re-entry of `int64(py_to<int64>(...))`.
5. [x] [ID: P0-CPP-S18-SAMECAST-02-S3-01] Confirm non-regression through regenerated diffs for sample/18 and transpile regressions.

### P0: Reduce unnecessary `rc` copies in sample/18 C++

Context: [docs/ja/plans/p0-cpp-s18-rc-copy-reduction.md](../plans/p0-cpp-s18-rc-copy-reduction.md)

1. [x] [ID: P0-CPP-S18-RC-COPY-02] Shift read-only `rc` paths in sample/18 to reference reception and reduce unnecessary copies.
2. [x] [ID: P0-CPP-S18-RC-COPY-02-S1-01] Inventory `rc` copy points (range-for / temporary bindings from subscripts) and define the conditions for reference conversion.
3. [x] [ID: P0-CPP-S18-RC-COPY-02-S2-01] Update C++ emitter output to reduce read-only paths to `const` references.
4. [x] [ID: P0-CPP-S18-RC-COPY-02-S2-03] Under safe conditions, reduce temporary `rc` bindings derived from subscript access to `const` references.
5. [x] [ID: P0-CPP-S18-RC-COPY-02-S2-02] Add regression tests to detect re-entry of `rc` value copies.
6. [x] [ID: P0-CPP-S18-RC-COPY-02-S3-01] Confirm non-regression by regenerating sample/18 and running transpile regressions.

### P0: Move sample/18 C++ class generation to initializer lists

Context: [docs/ja/plans/p0-cpp-s18-ctor-initlist.md](../plans/p0-cpp-s18-ctor-initlist.md)

1. [x] [ID: P0-CPP-S18-CTOR-INITLIST-01] Move synthetic constructors of generated sample/18 classes to initializer-list form.
2. [x] [ID: P0-CPP-S18-CTOR-INITLIST-01-S1-01] Inventory current constructor output and fix initializer-list targets and exclusion conditions.
3. [x] [ID: P0-CPP-S18-CTOR-INITLIST-01-S2-01] Change synthetic-constructor output in the class emitter to initializer-list form.
4. [x] [ID: P0-CPP-S18-CTOR-INITLIST-01-S2-02] Add regression tests to detect re-entry of body-assignment style.
5. [x] [ID: P0-CPP-S18-CTOR-INITLIST-01-S3-01] Confirm non-regression through regenerated diffs for sample/18 and transpile regressions.

### P0: Improve dynamic dispatch for inherited methods on non-C++ backends (all backends)

Context: [docs/ja/plans/p0-multilang-inheritance-dispatch-rollout.md](../plans/p0-multilang-inheritance-dispatch-rollout.md)

1. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01] Align inherited-method dynamic dispatch and `super()` with Python behavior across non-C++ backends.
2. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-JAVA] Execute the Java improvement plan [p0-java-inheritance-dispatch-uplift.md](../plans/p0-java-inheritance-dispatch-uplift.md).
3. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-JS] Execute the JS improvement plan [p0-js-inheritance-dispatch-uplift.md](../plans/p0-js-inheritance-dispatch-uplift.md).
4. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-TS] Execute the TS improvement plan [p0-ts-inheritance-dispatch-uplift.md](../plans/p0-ts-inheritance-dispatch-uplift.md).
5. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-KOTLIN] Execute the Kotlin improvement plan [p0-kotlin-inheritance-dispatch-uplift.md](../plans/p0-kotlin-inheritance-dispatch-uplift.md).
6. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-SWIFT] Execute the Swift improvement plan [p0-swift-inheritance-dispatch-uplift.md](../plans/p0-swift-inheritance-dispatch-uplift.md).
7. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-RS] Execute the Rust improvement plan [p0-rs-inheritance-dispatch-uplift.md](../plans/p0-rs-inheritance-dispatch-uplift.md).
8. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-RUBY] Execute the Ruby improvement plan [p0-ruby-inheritance-dispatch-uplift.md](../plans/p0-ruby-inheritance-dispatch-uplift.md).
9. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-LUA] Execute the Lua improvement plan [p0-lua-inheritance-dispatch-uplift.md](../plans/p0-lua-inheritance-dispatch-uplift.md).
10. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S3-01] Aggregate parity/smoke results across all backends and manage remaining blockers separately.

### P1: Redefine EAST2 as the first common IR (remove Python dependence)

Context: [docs/ja/plans/p1-east2-common-ir-depythonization.md](../plans/p1-east2-common-ir-depythonization.md)

1. [x] [ID: P1-EAST2-COMMON-IR-01] Redefine EAST2 as the first common IR and isolate Python-specific contracts at the frontend boundary.
2. [x] [ID: P1-EAST2-COMMON-IR-01-S1-01] Inventory Python dependencies in EAST2/EAST2->EAST3 lowering (`py_*` runtime calls, direct builtin-name references).
3. [x] [ID: P1-EAST2-COMMON-IR-01-S1-02] Document the EAST2 common-IR spec (nodes / operations / metadata / diagnostics / fail-closed).
4. [x] [ID: P1-EAST2-COMMON-IR-01-S2-01] Move Python-specific builtin/std resolution into the frontend adapter layer and separate it from the EAST2 contract.
5. [x] [ID: P1-EAST2-COMMON-IR-01-S2-02] Rewire `east2_to_east3_lowering.py` on a neutral-contract basis and reduce/remove Python-name branching.
6. [x] [ID: P1-EAST2-COMMON-IR-01-S2-03] Introduce a staged-migration bridge (temporary compatibility) to preserve non-regression for existing Python input.
7. [x] [ID: P1-EAST2-COMMON-IR-01-S3-01] Add regression tests that detect re-entry of Python-dependent contracts into EAST2.
8. [x] [ID: P1-EAST2-COMMON-IR-01-S3-02] Confirm non-regression on representative transpile/smoke/parity cases and document the migration result.
