            ...
            for (int64 dy = -(1); dy < 2; ++dy) {
                for (int64 dx = -(1); dx < 2; ++dx) {
                    if ((dx != 0) || (dy != 0)) {
                        int64 nx = (x + dx + w) % w;
                        int64 ny = (y + dy + h) % h;
                        cnt += py_list_at_ref(py_list_at_ref(rc_list_ref(grid), ny), nx);
                    }
                }
            }
            int64 alive = py_list_at_ref(py_list_at_ref(rc_list_ref(grid), y), x);
            if ((alive == 1) && ((cnt == 2) || (cnt == 3))) {
                rc_list_ref(row).append(1);
            } else if ((alive == 0) && (cnt == 3)) {
                rc_list_ref(row).append(1);
            } else {
                rc_list_ref(row).append(0);
            }
        }
        rc_list_ref(nxt).append(rc_list_copy_value(row));
    }
    return nxt;
}
...
bytes render(const rc<list<list<int64>>>& grid, int64 w, int64 h, int64 cell) {
    ...
    for (int64 y = 0; y < h; ++y) {
        for (int64 x = 0; x < w; ++x) {
            int64 v = (py_list_at_ref(py_list_at_ref(rc_list_ref(grid), y), x) ? 255 : 0);
...
void run_07_game_of_life_loop() {
        ...
        for (int64 gx = 8; gx < w - 8; gx += 22) {
            ...
            if (kind == 0) {
                ...
                for (int64 py = 0; py < ph; ++py) {
                    pw = (py_list_at_ref(rc_list_ref(glider), py)).size();
                    for (int64 px = 0; px < pw; ++px) {
                        if (py_list_at_ref(py_list_at_ref(rc_list_ref(glider), py), px) == 1)
                            py_list_at_ref(py_list_at_ref(rc_list_ref(grid), (gy + py) % h), (gx + px) % w) = 1;
                    }
                }
            } else if (kind == 1) {
                ph = (rc_list_ref(r_pentomino)).size();
                for (int64 py = 0; py < ph; ++py) {
                    pw = (py_list_at_ref(rc_list_ref(r_pentomino), py)).size();
                    for (int64 px = 0; px < pw; ++px) {
                        if (py_list_at_ref(py_list_at_ref(rc_list_ref(r_pentomino), py), px) == 1)
                            py_list_at_ref(py_list_at_ref(rc_list_ref(grid), (gy + py) % h), (gx + px) % w) = 1;
                    }
                }
            } else {
                ...
                for (int64 py = 0; py < ph; ++py) {
                    pw = (py_list_at_ref(rc_list_ref(lwss), py)).size();
                    for (int64 px = 0; px < pw; ++px) {
                        if (py_list_at_ref(py_list_at_ref(rc_list_ref(lwss), py), px) == 1)
                            py_list_at_ref(py_list_at_ref(rc_list_ref(grid), (gy + py) % h), (gx + px) % w) = 1;
                    }
                }
            }
    ...
    for (int64 _ = 0; _ < steps; ++_) {
        rc_list_ref(frames).append(render(grid, w, h, cell));
