...
function render_frame(width, height, center_x, center_y, scale, max_iter)
    ...
    local __hoisted_cast_1 = (tonumber(max_iter) or 0.0)
    for y = 0, (height) - 1, 1 do
        ...
        for x = 0, (width) - 1, 1 do
            ...
            frame[((((row_base + x)) < 0) and (#(frame) + ((row_base + x)) + 1) or (((row_base + x)) + 1))] = (math.floor(tonumber(((255.0 * i) / __hoisted_cast_1)) or 0))
            ::__pytra_continue_2::
        end
        ::__pytra_continue_1::
    end
