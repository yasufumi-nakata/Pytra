...
function palette()
    p = {}
    for i = 0, (256) - 1, 1 do
        r = math.min(255, (math.floor(tonumber((20 + (i * 0.9))) or 0)))
        g = math.min(255, (math.floor(tonumber((10 + (i * 0.7))) or 0)))
        b = math.min(255, (30 + i))
        table.insert(p, r)
        table.insert(p, g)
        table.insert(p, b)
...
function scene(x, y, light_x, light_y)
    ...
    v = (math.floor(tonumber((((255.0 * blob) * lit) * 5.0)) or 0))
...
function run_14_raymarching_light_cycle()
    ...
    start = perf_counter()
    local frames = {  }
    local __hoisted_cast_1 = (tonumber(frames_n) or 0.0)
    local __hoisted_cast_2 = (tonumber((h - 1)) or 0.0)
    local __hoisted_cast_3 = (tonumber((w - 1)) or 0.0)
