...
class Parser() {
    ...
    def expect(kind: String): Token = {
        ...
        if ((__pytra_str(token.kind) != __pytra_str(kind))) {
            throw new RuntimeException(__pytra_str(((__pytra_str((__pytra_str((__pytra_str((__pytra_str("parse error at pos=") + __pytra_str(__pytra_str(token.pos)))) + __pytra_str(", expected="))) + __pytra_str(kind))) + __pytra_str(", got=")) + token.kind)))
        }
        this.pos += 1L
    ...
    def parse_primary(): Long = {
        ...
        throw new RuntimeException(__pytra_str(((__pytra_str((__pytra_str("primary parse error at pos=") + __pytra_str(__pytra_str(t.pos)))) + __pytra_str(" got=")) + t.kind)))
        return 0L
    }
}
                    ...
                    while ((__pytra_int(i) < __pytra_int(n))) {
                        boundary:
                            ...
                            if ((__pytra_int(single_tag) > __pytra_int(0L))) {
                                tokens = __pytra_as_list(tokens); tokens.append(new Token(__pytra_str(__pytra_get_index(single_char_token_kinds, (__pytra_int(single_tag) - __pytra_int(1L)))), ch, i, 0L))
                                i += 1L
                                break(())(using __continueLabel_7)
                            }
                            if (__pytra_truthy(__pytra_isdigit(ch))) {
                                ...
                                tokens = __pytra_as_list(tokens); tokens.append(new Token("NUMBER", text, start, __pytra_int(text)))
                                break(())(using __continueLabel_7)
                            }
                            if ((__pytra_truthy(__pytra_isalpha(ch)) || (__pytra_str(ch) == __pytra_str("_")))) {
                                ...
                                if ((__pytra_str(text) == __pytra_str("let"))) {
                                    tokens = __pytra_as_list(tokens); tokens.append(new Token("LET", text, start, 0L))
                                } else {
                                    if ((__pytra_str(text) == __pytra_str("print"))) {
                                        tokens = __pytra_as_list(tokens); tokens.append(new Token("PRINT", text, start, 0L))
                                    } else {
                                        tokens = __pytra_as_list(tokens); tokens.append(new Token("IDENT", text, start, 0L))
                                    }
                                }
                                break(())(using __continueLabel_7)
                            }
                            throw new RuntimeException(__pytra_str((__pytra_str((__pytra_str((__pytra_str((__pytra_str((__pytra_str("tokenize error at line=") + __pytra_str(__pytra_str(line_index)))) + __pytra_str(" pos="))) + __pytra_str(__pytra_str(i)))) + __pytra_str(" ch="))) + __pytra_str(ch))))
                    }
                tokens = __pytra_as_list(tokens); tokens.append(new Token("NEWLINE", "", n, 0L))
            __i_1 += 1L
        }
    tokens = __pytra_as_list(tokens); tokens.append(new Token("EOF", "", __pytra_len(lines), 0L))
...
def build_benchmark_source(var_count: Long, loops: Long): mutable.ArrayBuffer[Any] = {
    ...
    boundary:
        ...
        while ((__step_2 >= 0L && i < __pytra_int(var_count)) || (__step_2 < 0L && i > __pytra_int(var_count))) {
            boundary:
                given __continueLabel_1: boundary.Label[Unit] = summon[boundary.Label[Unit]]
                lines = __pytra_as_list(lines); lines.append((__pytra_str((__pytra_str((__pytra_str("let v") + __pytra_str(__pytra_str(i)))) + __pytra_str(" = "))) + __pytra_str(__pytra_str((__pytra_int(i) + __pytra_int(1L))))))
            i += __step_2
        }
    i = __pytra_int(0L)
    boundary:
        ...
        while ((__step_5 >= 0L && i < __pytra_int(loops)) || (__step_5 < 0L && i > __pytra_int(loops))) {
            boundary:
