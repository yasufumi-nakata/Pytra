...
EAST3 スナップショットテスト（golden file と EAST3 出力の差分チェック）。

**モード:**

- **既定（引数なし）**: `test/fixture/` の EAST3 golden と再生成結果を比較する。
- **`--check-runtime-east`**: `src/runtime/east/` の `.east` ファイルが Python 正本（`src/pytra/{built_in,std,utils}/*.py`）から再生成した結果と一致するか検証する。`src/runtime/east/` は git 管理外の生成キャッシュであり、linker がマルチモジュール連結時に stdlib の型情報を解決するために参照する。

**共通フラグ:**

- `--update`: 差分がある場合に golden / `.east` ファイルを再生成して上書きする。

**使用例:**

```bash
# fixture golden の鮮度チェック
python3 tools/check/check_east3_golden.py

# runtime east の鮮度チェック
python3 tools/check/check_east3_golden.py --check-runtime-east

# runtime east を Python 正本から再生成
python3 tools/check/check_east3_golden.py --check-runtime-east --update
```
