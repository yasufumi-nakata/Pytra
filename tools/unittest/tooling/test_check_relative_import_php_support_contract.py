...
from tools.check_relative_import_php_support_contract import (
    EXPECTED_BACKEND,
    EXPECTED_HANDOFF,
    EXPECTED_SCENARIOS,
    EXPECTED_SMOKE,
...
class RelativeImportPhpSupportContractTest(unittest.TestCase):
    def test_validator_accepts_contract(self) -> None:
        validate_relative_import_php_support_contract()

    def test_scenarios_are_fixed(self) -> None:
        self.assertEqual(
            {entry["scenario_id"] for entry in RELATIVE_IMPORT_PHP_SUPPORT_SCENARIOS_V1},
            set(EXPECTED_SCENARIOS),
        )

    def test_backend_contract_is_fixed(self) -> None:
        self.assertEqual(RELATIVE_IMPORT_PHP_SUPPORT_BACKEND_V1, EXPECTED_BACKEND)

    def test_smoke_contract_is_fixed(self) -> None:
        self.assertEqual(RELATIVE_IMPORT_PHP_SUPPORT_SMOKE_V1, EXPECTED_SMOKE)

    def test_handoff_is_fixed(self) -> None:
        self.assertEqual(RELATIVE_IMPORT_PHP_SUPPORT_HANDOFF_V1, EXPECTED_HANDOFF)
