#!/usr/bin/env python3
"""Validate the backend-local Lua relative-import support contract."""
...
def validate_relative_import_lua_support_contract() -> None:
    scenario_map = {
        str(entry["scenario_id"]): entry for entry in RELATIVE_IMPORT_LUA_SUPPORT_SCENARIOS_V1
    }
    if set(scenario_map) != set(EXPECTED_SCENARIOS):
        raise SystemExit(
            "lua relative-import support scenarios drifted: "
    ...
    for scenario_id, expected in EXPECTED_SCENARIOS.items():
        ...
        for key, value in expected.items():
            if current[key] != value:
                raise SystemExit(
                    "lua relative-import support scenario drifted: "
                    f"{scenario_id}.{key}={current[key]!r} != {value!r}"
                )
    if RELATIVE_IMPORT_LUA_SUPPORT_BACKEND_V1 != EXPECTED_BACKEND:
        raise SystemExit("lua relative-import support backend contract drifted")
    if RELATIVE_IMPORT_LUA_SUPPORT_SMOKE_V1 != EXPECTED_SMOKE:
        raise SystemExit("lua relative-import support smoke contract drifted")
    if RELATIVE_IMPORT_LUA_SUPPORT_HANDOFF_V1 != EXPECTED_HANDOFF:
        raise SystemExit("lua relative-import support handoff drifted")
    if relative_import_lua_support_parent_backend_snapshot() != {
        "backend": "lua",
        ...
        raise SystemExit(
            "lua relative-import support parent backend snapshot drifted from the generic contract"
        )
    for rel_path in RELATIVE_IMPORT_LUA_SUPPORT_HANDOFF_V1["active_plan_paths"]:
        if not (ROOT / rel_path).is_file():
            raise SystemExit(f"missing lua support active plan path: {rel_path}")
    smoke_path = ROOT / RELATIVE_IMPORT_LUA_SUPPORT_SMOKE_V1["smoke_test_file"]
    if not smoke_path.is_file():
        raise SystemExit(f"missing lua support smoke file: {smoke_path}")
    ...
    for needle in (
        *RELATIVE_IMPORT_LUA_SUPPORT_SMOKE_V1["expected_error_family"],
        ...
        if needle not in emitter_src:
            raise SystemExit(f"missing lua emitter diagnostic marker: {needle}")


def main() -> None:
    validate_relative_import_lua_support_contract()
    print("[OK] lua relative-import support contract passed")
