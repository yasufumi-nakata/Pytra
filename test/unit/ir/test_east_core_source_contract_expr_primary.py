...
class EastCoreSourceContractExprPrimaryTest(unittest.TestCase):
    def test_primary_cluster_defs_live_in_split_module(self) -> None:
        ...
        self.assertNotIn("def _make_bin_impl(", core_text)
        self.assertNotIn("def _parse_primary_impl(", core_text)
        self.assertIn("from toolchain.ir.core_expr_primary import _make_bin_impl", core_text)
        self.assertIn("from toolchain.ir.core_expr_primary import _ShExprPrimaryParserMixin", core_text)

    def test_core_wrappers_delegate_primary_cluster_to_split_module(self) -> None:
        core_text = CORE_SOURCE_PATH.read_text(encoding="utf-8")

        make_bin_text = core_text.split("def _make_bin", 1)[1].split("def _sh_parse_expr", 1)[0]
