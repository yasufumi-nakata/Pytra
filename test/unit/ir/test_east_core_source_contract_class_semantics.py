...
class EastCoreSourceContractClassSemanticsTest(unittest.TestCase):
    def test_core_source_moves_class_semantics_helpers_out_of_core(self) -> None:
        ...
        self.assertIn("from toolchain.ir.core_class_semantics import _sh_collect_nominal_adt_class_metadata", core_text)
        self.assertIn("from toolchain.ir.core_class_semantics import _sh_is_value_safe_dataclass_candidate", core_text)
        self.assertIn("from toolchain.ir.core_class_semantics import _sh_make_decl_meta", core_text)
        self.assertIn("from toolchain.ir.core_class_semantics import _sh_make_nominal_adt_v1_meta", core_text)
