...
class EastCoreSourceContractImportModuleBuildersTest(unittest.TestCase):
    ...
    def test_core_source_routes_import_and_module_builders_through_split_module(self) -> None:
        ...
        self.assertIn("_sh_make_import_stmt(_sh_make_stmt_node, _sh_span(i, 0, len(ln)), aliases)", core_text)
        self.assertIn("_sh_make_import_from_stmt(\n                        _sh_make_stmt_node,", core_text)
        self.assertIn(
            "import_symbol_bindings[local_name] = _sh_make_import_symbol_binding(module_id, export_name)",
            core_text,
        )
        self.assertIn("qualified_symbol_refs.append(_sh_make_qualified_symbol_ref(module_id, export_name, local_name))", core_text)
        self.assertIn("out = _sh_make_module_root(", core_text)
        self.assertIn("make_node=_sh_make_node", core_text)
