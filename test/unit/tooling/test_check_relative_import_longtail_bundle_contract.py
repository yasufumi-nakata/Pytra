...
from tools.check_relative_import_longtail_bundle_contract import (
    EXPECTED_BACKENDS,
    EXPECTED_HANDOFF,
    EXPECTED_SCENARIOS,
...
class RelativeImportLongtailBundleContractTest(unittest.TestCase):
    def test_validator_accepts_contract(self) -> None:
        validate_relative_import_longtail_bundle_contract()

    def test_scenarios_are_fixed(self) -> None:
        self.assertEqual(
            {entry["scenario_id"] for entry in RELATIVE_IMPORT_LONGTAIL_BUNDLE_SCENARIOS_V1},
            set(EXPECTED_SCENARIOS),
        )

    def test_backend_order_and_lane_are_fixed(self) -> None:
        self.assertEqual(
            tuple(entry["backend"] for entry in RELATIVE_IMPORT_LONGTAIL_BUNDLE_BACKENDS_V1),
            EXPECTED_BACKENDS,
        )
        self.assertTrue(
            all(
    ...
    def test_handoff_is_fixed(self) -> None:
        self.assertEqual(RELATIVE_IMPORT_LONGTAIL_BUNDLE_HANDOFF_V1, EXPECTED_HANDOFF)
