...
func calc_17(values any) any {
    var total any = 0
    _ = total
    for _, __pytra_it_1 := range pyIter(values) {
        var v any = __pytra_it_1
        _ = v
        if (pyBool(pyEq(pyMod(v, 2), 0))) {
            total = pyAdd(total, v)
        } else {
            total = pyAdd(total, pyMul(v, 2))
        }
    }
    return total
}
