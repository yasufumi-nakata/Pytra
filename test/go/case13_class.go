...
var __cls_Multiplier = map[any]any{}
func NewMultiplier() map[any]any {
    self := map[any]any{"__class": "Multiplier"}
    return self
}
func Multiplier_mul(self map[any]any, x any, y any) any {
    return pyMul(x, y)
}

func main() {
    var m any = NewMultiplier()
    _ = m
    pyPrint(Multiplier_mul(m.(map[any]any), 6, 7))
}
