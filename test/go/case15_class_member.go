...
var __cls_Counter = map[any]any{"value": 0}
func NewCounter() map[any]any {
    self := map[any]any{"__class": "Counter"}
    return self
}
func Counter_inc(self map[any]any) any {
    pySet(__cls_Counter, "value", pyAdd(pyGet(__cls_Counter, "value"), 1))
    return pyGet(__cls_Counter, "value")
}

func main() {
    var c any = NewCounter()
    _ = c
    pyPrint(Counter_inc(c.(map[any]any)))
}
