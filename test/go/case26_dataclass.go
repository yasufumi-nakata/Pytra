...
var __cls_Point99 = map[any]any{}
func NewPoint99(__args ...any) map[any]any {
    self := map[any]any{"__class": "Point99"}
    var x any = nil
    if len(__args) > 0 { x = __args[0] }
    self["x"] = x
    var y any = 10
    if len(__args) > 1 { y = __args[1] }
    self["y"] = y
    return self
}
func Point99_total(self map[any]any) any {
    return pyAdd(pyGet(self, "x"), pyGet(self, "y"))
}

func main() {
    var p any = NewPoint99(3)
    _ = p
    pyPrint(Point99_total(p.(map[any]any)))
}
