...
var __cls_Point = map[any]any{}
func NewPoint(x any, y any) map[any]any {
    self := map[any]any{"__class": "Point"}
    pySet(self, "x", x)
    pySet(self, "y", y)
    return self
}
func Point_total(self map[any]any) any {
    return pyAdd(pyGet(self, "x"), pyGet(self, "y"))
}

func main() {
    var p any = NewPoint(2, 5)
    _ = p
    pyPrint(Point_total(p.(map[any]any)))
}
