...
## 2026-02-22 完了: `P0-SH-01` / `P0-SH-03` selfhost `.py` 直入力経路の回復

1. [x] `docs-ja/todo.md` の `ID: P0-SH-01` と `ID: P0-SH-03` を完了化した。
2. [x] root cause を修正した。
   - [x] `src/py2cpp.py` の `CppEmitter.__init__` で再初期化していた `import_modules` / `import_symbols` / `import_symbol_modules` を削除し、selfhost 生成 C++ で基底 helper が空テーブルを参照する経路を解消した。
3. [x] 回帰確認を実施した。
   - [x] `python3 tools/build_selfhost.py` の成功を確認した。
   - [x] `python3 tools/verify_selfhost_end_to_end.py --skip-build --cases sample/py/01_mandelbrot.py sample/py/04_monte_carlo_pi.py test/fixtures/control/if_else.py` で `failures=0` を確認した。
   - [x] `python3 tools/verify_selfhost_end_to_end.py --skip-build --cases sample/py/02_raytrace_spheres.py sample/py/06_julia_parameter_sweep.py sample/py/10_plasma_effect.py sample/py/11_lissajous_particles.py sample/py/14_raymarching_light_cycle.py sample/py/16_glass_sculpture_chaos.py` で `failures=0` を確認した。
   - [x] `python3 tools/check_selfhost_direct_compile.py --cases sample/py/*.py` で `failures=0` を確認した。
4. [x] 残課題の境界を明記した。
   - [x] `python3 tools/check_selfhost_cpp_diff.py --mode allow-not-implemented` は `mismatches=3`（`if_else.py`, `01_mandelbrot.py`, `04_monte_carlo_pi.py`）のままであり、差分縮退は `ID: P0-SH-04` 以降で継続する。
...
## 2026-02-22 完了: `docs-ja/todo.md` の P3/P4 一式消化
...
4. [x] `docs-ja/todo.md` を未完了専用の最小構成に整理した。
...
2. [x] `materials/refs/Yanesdk/docs/*/yanesdk.py` 重複配置の扱いを確定した。
   - [x] 調査/変換は canonical 正本 `materials/refs/Yanesdk/yanesdk/yanesdk.py` を優先する。
...
3. [x] import 仕様書へ重複配置除外ルールを明文化した。
   - [x] `docs-ja/spec-import.md` に `materials/refs/Yanesdk/docs/*/yanesdk.py` を重複コピーとして除外運用するルールを追記した。
...
10. [x] 文末 `;` は受理しない仕様へ変更し、エラー化した。
    - [x] `docs-ja/spec-user.md` に「文末 `;` 非対応」を追記した。
11. [x] 回帰テストを追加した。
...
1. [x] `docs-ja/todo.md` の `ID: DOCS-SYNC-01` を完了した。
2. [x] `docs/todo-history/` の運用導線を整備した。
   - [x] `tools/sync_todo_history_translation.py` を追加し、`docs-ja/todo-history/YYYYMMDD.md` を基点に `docs/todo-history/YYYYMMDD.md` の不足ファイル雛形を自動生成できるようにした。
...
3. [x] 文書ルールを同期した。
   - [x] `docs-ja/plans/archive/20260223-tg-docs-sync.md` に日次運用手順（同期・翻訳・check・diff確認）を追記した。
   - [x] `docs-ja/spec-tools.md` / `docs/spec-tools.md` に新規ツールの目的を追記した。

## 2026-02-22 完了: `P1-MQ-08` ゴールデン比較運用への切替

1. [x] `docs-ja/todo.md` の `ID: P1-MQ-08` を完了した。
...
1. [x] `docs-ja/todo.md` の `ID: P1-COMP-06` と `ID: P1-COMP-07` を完了した。
2. [x] `CodeEmitter` と EAST parser の境界を仕様へ明文化した。
   - [x] `docs-ja/spec-dev.md` に「`CodeEmitter` は EAST -> コード生成に専念し、FS 依存の import 解決/プロジェクト解析を持たない」境界を追記した。
   - [x] `docs-ja/spec-dev.md` に「EAST parser は単一ファイル EAST 化に専念し、依存グラフ解析責務を持たない」境界を追記した。
   - [x] `docs-ja/spec-dev.md` に compiler 共通層（`src/pytra/compiler/`）が FS 依存解析を担当する責務分担を追記した。
3. [x] 文脈ファイルへ判断を反映した。
   - [x] `docs-ja/plans/p1-compiler-shared-extraction.md` の決定ログに、境界明文化完了を追記した。

## 2026-02-22 完了: `P1-COMP-08` 他言語 CLI の共通解析 API 移行計画作成

1. [x] `docs-ja/todo.md` の `ID: P1-COMP-08` を完了した。
2. [x] `docs-ja/plans/p1-compiler-shared-extraction.md` に移行計画を追加した。
...
1. [x] `docs-ja/todo.md` の `ID: P1-RUNTIME-04` と `ID: P1-RUNTIME-06` を完了した。
2. [x] `P1-RUNTIME-04` として、Rust以外言語の移行計画を追加した。
   - [x] `docs-ja/plans/p1-runtime-layout-unification.md` に、`src/{cs,js,ts,go,java,swift,kotlin}_module/` の資産マップと `src/runtime/<lang>/pytra/` への段階移行（Step A-D）を追記した。
   - [x] 完了条件（非Rust runtime の新配置統一、`py2*`/hooks の新配置基準化）を明記した。
3. [x] `P1-RUNTIME-06` として、運用ルールを明文化した。
   - [x] `docs-ja/plans/p1-runtime-layout-unification.md` に「`src/*_module/` は互換レイヤ専用」「新規 runtime 実体追加禁止」の運用ルールを追記した。
   - [x] `docs-ja/spec-codex.md` の配置ルールを `src/runtime/<lang>/pytra/` 基準へ更新し、`src/*_module/` の新規実体追加禁止を追記した。
   - [x] `docs-ja/spec-dev.md` / `docs-ja/pytra-readme.md` に移行方針（`runtime/` 正本、`*_module/` 旧配置）を反映した。

## 2026-02-22 完了: `P0-SH-02` selfhost 最小実行経路の安定化（ネスト本文欠落修正）

1. [x] `docs-ja/todo.md` の `ID: P0-SH-02` を完了した。
2. [x] root cause を特定して修正した。
   - [x] selfhost C++ では仮想ディスパッチが無いため、`CodeEmitter` 既定の `emit_scoped_stmt_list` / `emit_scoped_block` 経路が base 実装へ静的束縛され、`if/for/while` のネスト本文が脱落することを確認した。
   - [x] `src/py2cpp.py` の `CppEmitter` に `emit_scoped_stmt_list` / `emit_scoped_block` override を追加し、selfhost 実行時も C++ 側文ディスパッチが `CppEmitter.emit_stmt` を通るように固定した。
3. [x] 回帰確認を実施した。
   - [x] `python3 tools/build_selfhost.py` の成功を確認した。
   - [x] `python3 tools/check_selfhost_cpp_diff.py --mode allow-not-implemented --cases test/fixtures/control/if_else.py --show-diff` で、差分が「本文欠落」から「brace 整形差」に縮退したことを確認した。
   - [x] `python3 tools/check_selfhost_cpp_diff.py --mode allow-not-implemented --cases sample/py/01_mandelbrot.py --show-diff` / `--cases sample/py/04_monte_carlo_pi.py --show-diff` で、差分が range-mode/brace などの整形差に限定されることを確認した。
   - [x] `python3 tools/verify_selfhost_end_to_end.py --skip-build --cases sample/py/01_mandelbrot.py sample/py/04_monte_carlo_pi.py test/fixtures/control/if_else.py` で `failures=0`（入力/生成/実行 end-to-end 一致）を確認した。
