...
<a href="../../../docs/todo/archive/20260224.md">
  <img alt="Read in English" src="https://img.shields.io/badge/docs-English-2563EB?style=flat-square">
</a>

## 2026-02-24 移管: `P0: py2cpp の EAST2 -> C++ 互換経路廃止`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/p0-cpp-east2-removal.md`（`TG-P0-CPP-EAST2-REMOVAL`）

1. [x] [ID: P0-CPP-EAST2-01] `py2cpp` の C++ 生成で `EAST2 -> C++` 互換経路を完全廃止し、EAST3 専用経路へ一本化する（`P0-CPP-EAST2-01-S1` から `P0-CPP-EAST2-01-S4` 完了でクローズ）。
2. [x] [ID: P0-CPP-EAST2-01-S1] `src/py2cpp.py` から `--east-stage 2` 受理経路を除去し、`EAST2` 指定時は明示エラーで停止する。
3. [x] [ID: P0-CPP-EAST2-01-S2] `src/py2cpp.py` 内の `EAST2 -> C++` 互換専用分岐（legacy fallback/互換ノード写像）を段階的に削除し、EAST3 前提の実装へ整理する。
...
## 2026-02-24 移管: `P0: EAST1 build 互換運用廃止`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/p0-east1-build-compat-removal.md`（`TG-P0-EAST1-BUILD-COMPAT-REMOVAL`）

1. [x] [ID: P0-EAST1-BUILD-01] `py2cpp` 起点の `EAST1 build/import graph` 互換運用を廃止し、`east_parts` 側の正規責務へ移管する（`P0-EAST1-BUILD-01-S1` から `P0-EAST1-BUILD-01-S4` 完了でクローズ）。
...
5. [x] [ID: P0-EAST1-BUILD-01-S4] `spec-east` / `spec-dev` と `py2cpp` 関連テストを同期し、責務境界と回帰ガードを固定する。
...
進捗メモ: [ID: P0-EAST1-BUILD-01-S4] `spec-east` / `spec-dev` に委譲境界を反映し、`test_east1_build.py`・`test_py2cpp_east1_build_bridge.py`・`check_py2cpp_transpile.py` で回帰ガードを固定した。

## 2026-02-24 移管: `P0: 依存解析の EAST1 build 責務化`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/p0-dependency-analysis-east1.md`（`TG-P0-DEP-ANALYSIS-EAST1`）

1. [x] [ID: P0-DEP-EAST1-01] 依存解析（import graph）の実装責務を `EAST1 build` 層へ集約し、`py2cpp.py` / `transpile_cli.py` から分離する（`P0-DEP-EAST1-01-S1` から `P0-DEP-EAST1-01-S4` 完了でクローズ）。
...
5. [x] [ID: P0-DEP-EAST1-01-S4] 依存解析責務境界を `spec-east` / `spec-dev` に明記し、`check_py2cpp_transpile` / smoke で回帰ガードを固定する。
...
進捗メモ: [ID: P0-DEP-EAST1-01-S4] `spec-east` / `spec-dev` に依存解析責務境界（`east1_build` 正本・`transpile_cli` thin wrapper）と回帰導線（unit + transpile check）を明記して固定。

## 2026-02-24 移管: `P0: EAST123 最終移行（段階境界表）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0-S1] `docs-ja/spec/spec-east.md` に各 stage の「入力/出力/禁止事項/担当ファイル」表を追加し、境界を仕様として固定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（CodeEmitter責務境界）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0-S2] `docs-ja/spec/spec-dev.md` に「CodeEmitter は EAST3 以降の構文写像専任、意味論 lower 禁止」を明記し、hooks との責務境界を固定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（段階横断棚卸し）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0-S3] `src/pytra/compiler/east_parts/` と `src/pytra/compiler/transpile_cli.py` の責務境界を棚卸しし、段階横断（EAST1/2/3 混在）の残存箇所を一覧化する。

## 2026-02-24 移管: `P0: EAST123 最終移行（段階境界ガード）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0-S4] stage 境界を破る実装（例: EAST2 での意味論 lower、CodeEmitter での再解釈）を検出するテスト/ガード（unit または tools）を追加する。

## 2026-02-24 移管: `P0: EAST123 最終移行（境界固定ゲート確定）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0-S5] `P0-EASTMIG-06-S0-S1` から `S0-S4` の結果を `docs-ja/plans/plan-east123-migration.md` に反映し、EAST 境界固定を `P0` の先行ゲートとして確定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（EAST2既定経路棚卸し）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S1] 全 `py2*.py` の EAST 読み込み経路を棚卸しし、`EAST2` 既定の経路（`load_east_document_compat` / `--east-stage 2` 既定）を一覧化する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2cpp既定 stage3 化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S2] `py2cpp.py` の既定を `--east-stage 3` に切替え、`--east-stage 2` は互換モードとして警告付き運用に縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（境界固定ゲート完了）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S0] （最優先）`EAST1/EAST2/EAST3` の責務境界を先に固定し、CodeEmitter が意味論処理を抱えない状態を受け入れ基準として明文化する（`P0-EASTMIG-06-S0-S1` から `P0-EASTMIG-06-S0-S5` 完了でクローズ）。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2rs EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S1] `py2rs.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2cs EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S2] `py2cs.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2js EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S3] `py2js.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2ts EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S4] `py2ts.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2go EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S5] `py2go.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2java EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S6] `py2java.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2kotlin EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S7] `py2kotlin.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（py2swift EAST3 主経路化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S8] `py2swift.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。

## 2026-02-24 移管: `P0: EAST123 最終移行（非 C++ 8変換器 契約統一）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3-S9] 非 C++ 8変換器の `--east-stage` 既定値・警告文言・回帰テスト導線を統一し、`EAST3` 主経路化の受け入れ条件を固定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（非 C++ 主経路化クローズ）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S3] 非 C++ 変換器（`py2rs.py`, `py2cs.py`, `py2js.py`, `py2ts.py`, `py2go.py`, `py2java.py`, `py2kotlin.py`, `py2swift.py`）で `EAST3` 主経路を導入し、`EAST2` 直依存を段階撤去する（`P0-EASTMIG-06-S3-S1` から `P0-EASTMIG-06-S3-S9` 完了でクローズ）。

## 2026-02-24 移管: `P0: EAST123 最終移行（EAST3 回帰導線既定化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S4] `test/unit/test_east3_*` と `tools/check_py2*_transpile.py` を更新し、`EAST3` 主経路が回帰導線の既定になるよう固定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（仕様同期: EAST2 互換モード）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S5] `docs-ja/spec/spec-east.md` と `docs-ja/spec/spec-dev.md` の記述を実装実態へ同期し、`EAST2` を「移行互換モード」として明文化する。

## 2026-02-24 移管: `P0: EAST123 最終移行（EAST1 build 境界正式化）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S6] `EAST1` build 責務境界を `docs-ja/spec/spec-east.md#east1-build-boundary` で正式化し、`load_east_document_compat` エラー契約互換・selfhost diff 実行・`EAST1` build での `EAST2` 非変換を受け入れ基準として固定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（EAST3 human renderer）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S7] （低優先）`east_parts/render_human_east2_cpp.py` と並行して `east_parts/render_human_east3_cpp.py` を追加し、`EAST3` 命令ノード（`ForCore`, `Box/Unbox`, `Obj*`, `type_id` 系）を人間可読ビューへ描画する経路を整備する。

## 2026-02-24 移管: `P0: EAST123 最終移行（selfhost差分切り分け）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06-S10] `check_selfhost_cpp_diff.py --mode allow-not-implemented` で検出された `sample/py/01_mandelbrot.py` と `sample/py/17_monte_carlo_pi.py` の差分原因を切り分け、`P0-EASTMIG-06` の最終クローズ条件を確定する。

## 2026-02-24 移管: `P0: EAST123 最終移行（クローズ）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

文脈: `docs-ja/plans/plan-east123-migration.md`（`TG-P0-EAST123-MIGRATION`）

1. [x] [ID: P0-EASTMIG-06] `EAST3` を全変換器の標準主経路として確定し、`EAST2` は明示互換モードに限定する（`P0-EASTMIG-06-S10` 完了でクローズ）。
...
## 2026-02-24 移管: `P1: 多言語出力品質（Swift/Kotlin preview 冗長縮退）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語出力品質（Swift/Kotlin preview 冗長縮退）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-02-S3-S3] Swift/Kotlin の node-backed 出力で import/base64 依存の冗長を縮退し、`sample/swift` / `sample/kotlin` の品質差分を縮小する。
2. [x] [ID: P1-MQ-02-S3] C#/Go/Java/Swift/Kotlin の cast/import 冗長を段階削減し、`sample/cpp` 比の差分を縮小する。

## 2026-02-24 移管: `P1: 多言語出力品質（再生成・再計測固定）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語出力品質（再生成・再計測固定）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-02-S4] 多言語サンプル再生成と品質再計測を行い、改善結果を定量比較として固定する。
2. [x] [ID: P1-MQ-02] 各言語 emitter/hooks/profile に段階的改善を入れ、`sample/cpp` と同等の可読性水準へ引き上げる（`P1-MQ-02-S1` から `P1-MQ-02-S4` 完了でクローズ）。

## 2026-02-24 移管: `P1: 多言語出力品質（回帰チェック導線整備）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語出力品質（回帰チェック導線整備）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-03] 多言語の出力品質回帰を防ぐ検査（品質指標 + transpile/smoke）を整備する。

## 2026-02-24 移管: `P1: 多言語 selfhost 検証（stage1 棚卸し）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語 selfhost 検証（stage1 棚卸し）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-04-S1] 非 C++ 各言語の stage1 self-transpile を実行し、preview/実行可否の初回ステータスを記録する。

## 2026-02-24 移管: `P1: 多言語 selfhost 検証（non-preview stage2 経路整備）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語 selfhost 検証（non-preview stage2 経路整備）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-04-S2] 非 preview 言語（`rs/cs/js`）の stage2 実行経路を整備し、生成された変換器で `sample/py` の再変換可否を固定する。
2. [x] [ID: P1-MQ-04] 非 C++ 各言語（`rs/cs/js/ts/go/java/swift/kotlin`）で、`py2<lang>.py` の selfhost 可否（自己変換した生成物で `sample/py` を再変換できるか）を検証し、言語別ステータスを記録する（`P1-MQ-04-S1` から `P1-MQ-04-S2` 完了でクローズ）。

## 2026-02-24 移管: `P1: 多言語 selfhost 検証（多段自己変換分類）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語 selfhost 検証（多段自己変換分類）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-05] 非 C++ 各言語で、生成物による再自己変換（多段 selfhost）が成立するかを検証し、失敗要因を分類する。

## 2026-02-24 移管: `P1: 多言語 selfhost 検証（定期実行導線）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語 selfhost 検証（定期実行導線）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-06] 非 C++ 言語の selfhost / 多段 selfhost 検証を定期実行できるチェック導線（手順またはスクリプト）を整備する。

## 2026-02-24 移管: `P1: 多言語 selfhost 検証（sample 再生成差分ゼロ）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P1: 多言語 selfhost 検証（sample 再生成差分ゼロ）

文脈: `docs-ja/plans/p1-multilang-output-quality.md`（`TG-P1-MULTILANG-QUALITY`）

1. [x] [ID: P1-MQ-07] `sample/` 生成物はタイムスタンプ埋め込みなしで管理し、CI で再生成差分ゼロ（常に最新）を必須化する。

## 2026-02-24 移管: `P3: microgpt 原本保全（compile 互換完了）`

以下を `docs-ja/todo/index.md` から `todo/archive` へ移管。

## P3: microgpt 原本保全（compile 互換完了）

文脈: `docs-ja/plans/p3-microgpt-source-preservation.md`（`TG-P3-MICROGPT-SOURCE-PRESERVATION`）

1. [x] [ID: P3-MSP-03] `work/tmp/microgpt-20260222-lite.py` 依存を縮退し、原本 `materials/refs/microgpt/microgpt-20260222.py` で transpile -> `g++ -fsyntax-only` が通る回帰導線を整備する。
