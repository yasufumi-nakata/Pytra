...
3. [ ] [ID: P0-EASTMIG-06-S0-S1] `docs-ja/spec/spec-east.md` に各 stage の「入力/出力/禁止事項/担当ファイル」表を追加し、境界を仕様として固定する。
4. [ ] [ID: P0-EASTMIG-06-S0-S2] `docs-ja/spec/spec-dev.md` に「CodeEmitter は EAST3 以降の構文写像専任、意味論 lower 禁止」を明記し、hooks との責務境界を固定する。
5. [ ] [ID: P0-EASTMIG-06-S0-S3] `src/pytra/compiler/east_parts/` と `src/pytra/compiler/transpile_cli.py` の責務境界を棚卸しし、段階横断（EAST1/2/3 混在）の残存箇所を一覧化する。
6. [ ] [ID: P0-EASTMIG-06-S0-S4] stage 境界を破る実装（例: EAST2 での意味論 lower、CodeEmitter での再解釈）を検出するテスト/ガード（unit または tools）を追加する。
7. [ ] [ID: P0-EASTMIG-06-S0-S5] `P0-EASTMIG-06-S0-S1` から `S0-S4` の結果を `docs-ja/plans/plan-east123-migration.md` に反映し、EAST 境界固定を `P0` の先行ゲートとして確定する。
8. [ ] [ID: P0-EASTMIG-06-S1] 全 `py2*.py` の EAST 読み込み経路を棚卸しし、`EAST2` 既定の経路（`load_east_document_compat` / `--east-stage 2` 既定）を一覧化する。
9. [ ] [ID: P0-EASTMIG-06-S2] `py2cpp.py` の既定を `--east-stage 3` に切替え、`--east-stage 2` は互換モードとして警告付き運用に縮退する。
10. [ ] [ID: P0-EASTMIG-06-S3] 非 C++ 変換器（`py2rs.py`, `py2cs.py`, `py2js.py`, `py2ts.py`, `py2go.py`, `py2java.py`, `py2kotlin.py`, `py2swift.py`）で `EAST3` 主経路を導入し、`EAST2` 直依存を段階撤去する（`P0-EASTMIG-06-S3-S1` から `P0-EASTMIG-06-S3-S9` 完了でクローズ）。
11. [ ] [ID: P0-EASTMIG-06-S3-S1] `py2rs.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
12. [ ] [ID: P0-EASTMIG-06-S3-S2] `py2cs.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
13. [ ] [ID: P0-EASTMIG-06-S3-S3] `py2js.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
14. [ ] [ID: P0-EASTMIG-06-S3-S4] `py2ts.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
15. [ ] [ID: P0-EASTMIG-06-S3-S5] `py2go.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
16. [ ] [ID: P0-EASTMIG-06-S3-S6] `py2java.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
17. [ ] [ID: P0-EASTMIG-06-S3-S7] `py2kotlin.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
18. [ ] [ID: P0-EASTMIG-06-S3-S8] `py2swift.py` の既定経路を `EAST3` 主経路へ切り替え、`EAST2` 既定読み込みを互換モードへ縮退する。
19. [ ] [ID: P0-EASTMIG-06-S3-S9] 非 C++ 8変換器の `--east-stage` 既定値・警告文言・回帰テスト導線を統一し、`EAST3` 主経路化の受け入れ条件を固定する。
20. [ ] [ID: P0-EASTMIG-06-S4] `test/unit/test_east3_*` と `tools/check_py2*_transpile.py` を更新し、`EAST3` 主経路が回帰導線の既定になるよう固定する。
