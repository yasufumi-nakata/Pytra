...
- 2026-02-27: `P0-EAST3-OPT-01-S3-02` として `runtime_parity_check.py` に `--east3-opt-level` を追加し、`sample/py` 18件 × `cpp,rs,cs,js,ts` を `O0/O1/O2` で再実行。3レベルとも `17 pass / 1 fail`（既知 `18_mini_language_interpreter:cpp` コンパイル失敗）で、opt level 切替による新規差分が無いことを `work/logs/east3_opt_parity_o{0,1,2}.json` で確認した。
...
- [x] [ID: P0-EAST3-OPT-01-S3-02] `sample` 回帰 + parity 検証を実行し、`O0`/`O1`/`O2` 切替時の互換を確認する。
