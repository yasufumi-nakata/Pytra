...
<a href="../../docs/plans/archive/pytra-wip.md">
...
- `pathlib`
  ...
  - 実装位置:
    ...
    - Rust: `src/runtime/rs/pytra/built_in/py_runtime.rs`（`PyPath`）
    ...
    - Kotlin: preview emitter（最小エントリ + 中間コードコメント形式）
  - 差分:
...
- ターゲット言語ごとのランタイム:
  ...
  - 旧配置（移行中）: `src/js_module`, `src/ts_module`
