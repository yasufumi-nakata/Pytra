...
- 2026-02-26: `P0-SAMPLE-CPP-RS-PERF-01-S16-01` として `python3 tools/benchmark_sample_cpp_rs.py --warmup 1 --repeat 5 --emit-json work/perf/cpp_rs_s15_all_retry_20260226.json` を完走し、18件の最新中央値を `readme.md` / `readme-ja.md` の C++/Rust 列へ反映した。主要値は `01: 0.748/0.741`, `09: 0.572/0.614`, `11: 0.332/0.336`, `16: 0.697/0.228`, `18: 0.340/0.386`。
- 2026-02-26: `P0-SAMPLE-CPP-RS-PERF-01-S16-02` として最新18件の外れ値判定を更新した。`>1.5x` は `16_glass_sculpture_chaos`（`rs/cpp=0.33`, `cpp/rs≈3.06`）のみで、`01/04/09/18` を含む従来外れ値は解消した。次段は `16` の C++ GIF path（encode と出力前整形）に限定して分解する。
- 2026-02-26: `P0-SAMPLE-CPP-RS-PERF-01-S17-01` として `src/hooks/cpp/emitter/cpp_emitter.py` と `src/hooks/cpp/emitter/operator.py` を更新し、if-join 事前宣言の型推論を強化した。`_infer_numeric_expr_type`（`math.sin/cos/max/min` 含む）と branch 内逐次型文脈を導入し、`get_expr_type` は Name の宣言済み型を優先するように修正、`Mod` は非整数オペランド時に `py_mod` へフォールバックするよう変更した。`16_glass_sculpture_chaos` 再生成 C++ で `make_object(` 出現が `14 -> 0`（`cx/cz` 退化修正後も `0`）を確認し、`python3 tools/check_py2cpp_transpile.py`（`checked=133 ok=133 fail=0 skipped=6`）と `python3 tools/runtime_parity_check.py --case-root sample --targets cpp --ignore-unstable-stdout 01_mandelbrot 09_fire_simulation 16_glass_sculpture_chaos 18_mini_language_interpreter`（`cases=4 pass=4 fail=0`）で回帰なしを確認した。
- 2026-02-26: `P0-SAMPLE-CPP-RS-PERF-01-S17-02` として再計測を実施した。`python3 tools/benchmark_sample_cpp_rs.py 16_glass_sculpture_chaos --warmup 1 --repeat 7 --emit-json work/perf/cpp_rs_s17_16_recheck_20260226.json` で `16: cpp=0.256 rs=0.227 (0.89x)`、`python3 tools/benchmark_sample_cpp_rs.py --warmup 1 --repeat 5 --emit-json work/perf/cpp_rs_s17_all_20260226.json` で18件を再計測し、最大乖離は `06` の `1.35x`（`max(cpp/rs, rs/cpp)`）で `>1.5x` 外れ値は消滅した。最新版中央値は `01:0.751/0.740`, `09:0.569/0.612`, `16:0.260/0.227`, `18:0.335/0.386` で、README（日英）へ反映した。
