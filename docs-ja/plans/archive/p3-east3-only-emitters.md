...
- 2026-02-26: `S7-02` として `runtime_parity_check --case-root sample --targets rs,cs,js,ts,go,java,swift,kotlin --all-samples --ignore-unstable-stdout` を実行。`SUMMARY cases=18 pass=18 fail=0`・`ok: 144` を確認し、非C++ 8ターゲットの `EAST3 only` 整合確認を完了した。
...
- [x] [ID: P3-EAST3-ONLY-01-S7-02] `runtime_parity_check --case-root sample --targets rs,cs,js,ts,go,java,swift,kotlin --all-samples --ignore-unstable-stdout` を実行し、整合を最終確認する。
